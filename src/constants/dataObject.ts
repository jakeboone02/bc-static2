import { IRecipe } from "../types/interfaces";

const recipes: {[key: string]: IRecipe} = {
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi20ggM: {
    status: 'active',
    subtitle: "Gabriel's 5th Birthday Cake",
    canEmailOwner: true,
    isFavorite: false,
    title: 'Apple Birthday Cake',
    lastModified: '2012-02-24T04:10:27',
    notes:
      'You can also bake the cake in a 13-by-9-inch pan; the baking time will be about the same. Also, if the buttercream separates, continue beating -- it will come together.',
    ingredients: [
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'For The Buttercream (or make a cream cheese frosting)',
        isGroupHeader: true,
      },
      {
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'large egg whites',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'packed light-brown sugar',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      {
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: '(3 sticks) unsalted butter, room temperature, cut into pieces',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'For The Cake', isGroupHeader: true },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: '(1 stick)unsalted butter, melted, plus more for pans',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour (spooned and leveled), plus more for pans',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'ground cinnamon',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground ginger',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'packed light-brown sugar',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'Granny Smith apples, peeled, two coarsely grated and two diced',
        isGroupHeader: false,
      },
    ],
    categories: 'Cake, apple',
    yield: 'Serves 12',
    equipment: '',
    source: 'Martha Stewart / Everyday Food, September 2008',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi20ggM',
    directions:
      'Back to Layer Cakes\nJohn\'s Three-Layer Apple Cake\nImagine those caramel apples you loved as a kid, now all grown up and baked into a cake. Stacks up well, doesn\'t it? Try this cake, courtesy of John Barricelli, host of "Everyday Baking."\n\n\nPreheat oven to 350 degrees. Butter three 8-inch-round cake pans; line bottoms with parchment paper. Butter, then flour paper and sides (tapping out excess); set aside.\n\nIn a medium bowl, whisk together flour, baking soda, baking powder, cinnamon, ginger, and salt; set aside. In a large bowl, whisk together butter, sugar, and eggs until well combined; fold in grated and diced apples. Add flour mixture; mix just until combined. Divide batter among prepared pans; smooth tops.\n\nBake until a toothpick inserted in center of cake comes out clean, 35 to 40 minutes. Cool cakes in pans 20 minutes, then invert onto wire racks; peel off parchment. Invert cakes again, and let cool completely on racks.\n\nMake buttercream: In a heatproof bowl set over (not in) a pan of simmering water, whisk together egg whites, sugar, and salt until sugar has dissolved; remove from heat. Using an electric mixer on medium, beat mixture until fluffy and cooled, about 15 minutes. Increase to high; beat until stiff peaks form. Reduce to medium-low; gradually add butter, beating until fully incorporated, about 3 to 5 minutes.\n\nAssemble cake: Place one layer on a serving plate; spread top with buttercream. Top with another layer; spread top with buttercream. Top with third layer; spread remaining buttercream over top and sides of cake. Refrigerate at least 1 hour (or up to 4 days). To serve, bring cake to room temperature; slice with a serrated knife.',
    preptime: '45 minutes\nTotal Time\n2 hours 20 minutes, plus chilling',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOhgoM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Banana Bread',
    lastModified: '2013-10-06T14:33:13',
    notes: '',
    ingredients: [
      { qty: 3, qty2: 4, uom: '', desc: 'ripe bananas', isGroupHeader: false },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'melted butter',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'sugar', isGroupHeader: false },
      { qty: 1.5, qty2: 0, uom: 'cups', desc: 'flour', isGroupHeader: false },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: '', desc: 'egg, beaten', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Chocolate chips (as many as you want!)',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOhgoM',
    directions:
      'Preheat oven to 350°F. Mash the bananas in a bowl. Add all the remaining ingredients and mix well with a wooden spoon. Bake in a buttered loaf pan until a toothpick stuck into the bread comes out clean, 55 to 60 minutes. Slice and serve.\n\nCookie, March 2006\n',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiG5QcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Banana Bread -- No Butter!',
    lastModified: '2011-09-08T01:58:53',
    notes: '',
    ingredients: [
      {
        qty: 3,
        qty2: 0,
        uom: '',
        desc: '(over) ripe bananas',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0.75,
        uom: 'cups',
        desc: 'sugar (the less ripe the bananas, the more sugar you will need)',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: '', desc: 'eggs', isGroupHeader: false },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc:
          'all purpose flour (I often replace one of the cups with whole wheat flour)',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'tsp', desc: 'salt', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'tsp', desc: 'vanilla', isGroupHeader: false },
      { qty: 0.5, qty2: 0, uom: 'tsp', desc: 'cinnamon', isGroupHeader: false },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'BreadandButter.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiG5QcM',
    directions:
      'Mash the bananas in a large bowl and add the sugar. The sugar will help liquefy the mixture.  Whisk in the eggs, and then sift in the dry ingredients. Still until evenly combined. Bake in a greased a floured loaf pan at 325 for about 1 hour or until a cake tester comes out clean. Allow to stand and cool for 15 minutes before removing from the pan.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhivugMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Beer Batter Bread',
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      { isGroupHeader: false, qty: 3, qty2: 0, uom: 'c.', desc: 'flour' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'T',
        desc: 'baking powder',
      },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: 'T', desc: 'sugar' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp.', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: 'oz.',
        desc: 'beer (room temperature)',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'c.',
        desc: 'unsalted butter, melted',
      },
    ],
    categories: 'breads',
    yield: '1 loaf',
    equipment: '',
    source: 'New Tastes from Texas, Epicurious',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhivugMM',
    directions:
      '1.  Preheat oven to 375°.  Brush a little butter into 9x5x3" pan.\n\n2.  Combine all dry ingredients.\n\n3.  Add beer, mixing as little as possible -- the batter will be lumpy and even have dry spots.\n\n4.  Pour into a pan and brush with remaining butter.\n\n5.  Bake 35-40 minutes or until skewer comes out clean.  This recipe makes one loaf, but I often double it and make two regular loaves or a few small ones.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj2jQoM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Best Banana Bread!',
    lastModified: '2013-10-06T14:52:54',
    notes: '',
    ingredients: [
      {
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'to 5 ripe bananas',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'melted butter',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'sugar', isGroupHeader: false },
      { qty: 1.5, qty2: 0, uom: 'cups', desc: 'flour', isGroupHeader: false },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: '',
        desc: 'to 1 tsp cinnamon',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: '', desc: 'egg, beaten', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Chocolate chips (as many as you want!)',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Walnuts', isGroupHeader: false },
      { qty: 0, qty2: 0, uom: '', desc: 'Pecans on top', isGroupHeader: false },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Sanding sugar on top!',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Cookie, March 2006',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj2jQoM',
    directions:
      'Preheat oven to 350°F. Mash the bananas in a bowl. Add all the remaining ingredients and mix well with a wooden spoon. Bake in a buttered loaf pan until a toothpick stuck into the bread comes out clean, 55 to 60 minutes. Slice and serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi-_AcM': {
    status: 'active',
    subtitle: 'Like Panera, easily doubles',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Broccoli Cheese Soup',
    lastModified: '2011-11-30T02:08:16',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'melted butter',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: '',
        desc: 'medium chopped onions',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'melted butter',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'cup', desc: 'flour' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'half-and-half cream',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chicken stock',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'lb',
        desc: 'fresh broccoli',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'carrots, julienned',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'nutmeg',
      },
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: 'ounces',
        desc: 'grated sharp cheddar cheese',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'salt and pepper',
      },
    ],
    categories: 'Soup, cheese, vegetables, starter',
    yield: '4',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi-_AcM',
    directions:
      "Prep Time: 30 mins\nTotal Time: 1 hrs 10 mins\nServings: 4\n\nSauté onion in butter. Set aside.\n\nCook melted butter and flour using a whisk over medium heat for 3-5 minutes. Stir constantly and add the half & half.\n\nAdd the chicken stock. Simmer for 20 minutes.\n\nAdd the broccoli, carrots and onions. Cook over low heat 20-25 minutes.\n\nAdd salt and pepper. Can be puréed in a blender but I don't. Return to heat and add cheese. Stir in nutmeg.\n\nNutrition Facts\nServing Size: 1 (437 g)\nServings Per Recipe: 4\n\nAmount Per Serving% Daily ValueCalories 628.1 Calories from Fat 44170%Amount Per Serving% Daily ValueTotal Fat 49.0g75%Saturated Fat 30.2g151%Cholesterol 146.1mg48%Sugars 5.5 gSodium 751.2mg31%Total Carbohydrate 25.4g8%Dietary Fiber 3.6g14%Sugars 5.5 g22%Protein 23.7g47%\n",
    preptime: '1 hour',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOiQkM: {
    status: 'active',
    subtitle:
      'Chocolate-Peanut Butter Cake with Cream Cheese and Butterfinger Frosting',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Butterfinger Cake',
    lastModified: '2012-03-28T01:41:40',
    notes: '',
    ingredients: [
      { qty: 0, qty2: 0, uom: '', desc: 'Filling:', isGroupHeader: true },
      {
        qty: 2.25,
        qty2: 0,
        uom: 'cups',
        desc: 'heavy whipping cream',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: '(packed) golden brown sugar',
        isGroupHeader: false,
      },
      {
        qty: 12,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet or semisweet chocolate, finely chopped',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'old-fashioned (natural) chunky peanut butter',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Cake:', isGroupHeader: true },
      {
        qty: 2.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      {
        qty: 10,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(1 1/4 sticks) unsalted butter, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'old-fashioned (natural) chunky peanut butter',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'pound',
        desc: 'golden brown sugar',
        isGroupHeader: false,
      },
      { qty: 4, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'buttermilk', isGroupHeader: false },
      { qty: 0, qty2: 0, uom: '', desc: 'Frosting:', isGroupHeader: true },
      {
        qty: 1.5,
        qty2: 0,
        uom: '',
        desc: '8-ounce packages cream cheese, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'powdered sugar, divided',
        isGroupHeader: false,
      },
      {
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(3/4 stick) unsalted butter, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'chilled heavy whipping cream',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Butterfinger candy bars, coarsely chopped',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Glazed peanuts',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit, March 2005',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOiQkM',
    directions:
      'For filling:\nBring cream and sugar to simmer in saucepan, whisking to dissolve sugar. Remove from heat. Add chocolate; let stand 1 minute. Whisk until smooth. Whisk in peanut butter. Chill uncovered overnight.\n\nFor cake:\nPreheat oven to 350°F. Butter three 9-inch-diameter cake pans with 1 1/2-inch-high sides. Line bottoms with parchment paper. Sift first 4 ingredients into medium bowl. Using electric mixer, beat butter and peanut butter in large bowl until blended. Beat in sugar. Beat in eggs, 1 at a time, then vanilla. At low speed, beat in flour mixture in 4 additions alternately with buttermilk in 3 additions.\n\nDivide batter among pans and spread evenly. Bake cakes until tester inserted into center comes out clean, about 25 minutes. Cool cakes 5 minutes. Turn out onto racks; peel off parchment. Cool cakes completely.\n\nFor frosting:\nUsing electric mixer, beat cream cheese, 1 1/4 cups powdered sugar, butter, and vanilla in large bowl to blend. Whisk whipping cream and 3/4 cup powdered sugar in bowl until mixture holds medium-firm peaks. Fold into cream cheese mixture in 3 additions; chill until firm but spreadable, about 1 hour.\n\nPlace 1 cake layer, bottom side up, on 9-inch tart pan bottom. Spread with half of filling. Place another layer, bottom side up, on work surface. Spread with remaining filling; place atop first layer. Top with remaining cake layer, bottom side up.\n\nSpread frosting over top and sides of cake. (Can be made 1 day ahead. Cover with cake dome; chill. Let stand at room temperature 2 hours before continuing.) Press candy and peanuts onto top of cake.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj9vQcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Buttermilk Biscuits',
    lastModified: '2011-06-19T19:34:58',
    notes: '',
    ingredients: [
      {
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
        isGroupHeader: false,
      },
      {
        qty: 4,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'teaspoon', desc: 'salt', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: '(1 1/2sticks) chilled unsalted butter, cut into 1/4-inch pieces',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'buttermilk', isGroupHeader: false },
    ],
    categories: 'Bread, breakfast',
    yield: '12 - 16',
    equipment: '',
    source: 'epicurious.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj9vQcM',
    directions:
      'Preheat oven to 425°F. Whisk flour, sugar, baking powder, salt and baking soda in large bowl to blend. Using fingertips, rub 3/4 cup chilled butter into dry ingredients until mixture resembles coarse meal. Add buttermilk and stir until evenly moistened. Using 1/4 cup dough for each biscuit, drop biscuits onto baking sheet, spacing 2 inches apart. Bake until biscuits are golden brown on top, about 15 minutes. Cool slightly. Serve warm.',
    preptime: '30 minutes',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhju6QgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Buttery Barley Risotto',
    lastModified: '2012-03-01T23:34:55',
    notes: '',
    ingredients: [
      {
        qty: 6.5,
        qty2: 0,
        uom: 'cups',
        desc: 'low-salt chicken broth',
        isGroupHeader: false,
      },
      {
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'unsalted European-style butter',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'medium onion, finely chopped',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'medium garlic cloves, minced',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'pearl barley (about 13 ounces), rinsed, drained',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc:
          '6-ounce package 3-inch-diameter portobello mushrooms, stemmed, gills scraped, caps sliced',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'freshly grated Parmesan cheese (about 1 1/2 oz.)',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'chopped fresh chives',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'chopped fresh Italian parsley',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'chopped fresh basil',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: '(May add grilled chicken or steamed vegetables as desired)',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Epicurious / Bon Appétit, 2003',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhju6QgM',
    directions:
      'Bring broth to simmer in medium saucepan. Reduce heat to low and cover to keep warm.\n\nMelt 2 tablespoons butter in heavy large saucepan over med heat. Add onion and garlic; sauté until tender, about 6 minutes. \n\nAdd barley; stir until coated with butter, about 1 minute. Add 1/2 cup broth; simmer, stirring often, until broth is absorbed, about 3 minutes. Add remaining broth 1/2 cup at a time, allowing broth to be absorbed before adding more and stirring frequently until barley is tender but still firm to bite and risotto is creamy, about 45 minutes.\n\nMeanwhile, melt 2 tablespoons butter in medium skillet over medium-high heat. Add mushrooms and sauté until soft, about 5 minutes. Season to taste with salt and pepper. Add mushrooms, Parmesan cheese, herbs, and remaining 2 tablespoons butter to risotto; stir to combine. Season to taste with salt and pepper. Divide risotto among bowls and serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjOyggM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Cauliflower and Caramelized Onion Tart',
    lastModified: '2012-02-22T22:11:50',
    notes: '',
    ingredients: [
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'small head of cauliflower (about 1 lb.), cored, 1” pieces',
        isGroupHeader: false,
      },
      {
        qty: 2.5,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'olive oil, divided',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'truffle oil',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'refrigerated pie crust',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large onion, halved lengthwise, thinly sliced',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'Dijon mustard',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc:
          '(7- to 8-ounce) container mascarpone cheese (Italian cream cheese)',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'whipping cream',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground white pepper',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Pinch of ground nutmeg',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'grated Gruyère cheese',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'grated Parmesan cheese',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: 'Serves 8',
    equipment: '',
    source: 'Bon Appétit, March 2007',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjOyggM',
    directions:
      'Position rack in center of oven; preheat to 425°F. Toss cauliflower with 1 tablespoon olive oil in large bowl. Spread on large rimmed baking sheet, spacing apart. Sprinkle with salt and pepper. Roast 15 minutes; turn florets over. Continue roasting until tender, about 25 minutes longer. Cool cauliflower, then thinly slice. Drizzle with truffle oil; toss. Reduce oven temperature to 350°F.\n\nPress pie crust onto bottom and up sides of 9-inch-diameter tart pan with removable bottom. Line pie crust with foil; fill with pie weights. Bake crust 20 minutes. Remove foil and pie weights; bake until crust is golden, about 5 minutes, pressing crust with back of fork if bubbles form. Cool crust. Maintain oven temperature.\n\nHeat remaining 1 1/2 tablespoons olive oil in heavy large skillet over medium heat. Add onion; sprinkle with salt and pepper. Cook until onion is deep golden brown, stirring occasionally, about 40 minutes. Cool slightly. DO AHEAD Can be made 1 day ahead. Store crust at room temperature. Cover and chill cauliflower and onion separately.\n\nBrush bottom and sides of crust with mustard. Spread onion in crust. Arrange cauliflower evenly over. Set tart on rimmed baking sheet. Whisk eggs and next 4 ingredients in medium bowl. Stir in Gruyère. Pour mixture over filling in tart pan; sprinkle with Parmesan. Bake until tart is golden and center is set, about 40 minutes. Transfer to rack; cool 15 minutes before serving.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj2kwgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Cafe Con Leche Cheesecake',
    lastModified: '2012-01-13T17:25:08',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '9-ounce box chocolate wafer cookies, broken',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'instant espresso powder',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet or semisweet chocolate, chopped',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(3/4 stick) unsalted butter, melted, hot',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Filling:' },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: '8-ounce packages cream cheese, room temperature',
      },
      { isGroupHeader: false, qty: 1.25, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'coffee liqueur',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'instant espresso powder',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'vanilla extract',
      },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet or semisweet chocolate, chopped',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Topping:' },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc: 'sour cream',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'instant espresso powder or instant coffee powder',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Garnish:' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'ounces',
        desc:
          'bittersweet (not unsweetened) or semisweet chocolate, chopped, melted',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Lightly sweetened whipped cream',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Coffee bean candies (optional)',
      },
    ],
    categories: 'Cheesecake, dessert, chocolate',
    yield: '9" cheesecake',
    equipment: 'Springform',
    source: 'Epicurious/Bon Appetit, May 2009',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj2kwgM',
    directions:
      'For crust: \nFinely grind cookies, sugar and espresso powder in processor. Add chocolate and chop finely. Add butter and blend until moist crumbs form. Press crumbs onto bottom and up sides of 9-inch-diameter springform pan with 2 3/4-inch-high sides.\n\n\nFor filling: \nPreheat oven to 350°F. Using electric mixer, beat cream cheese and sugar in large bowl until well blended. \n\nCombine coffee liqueur and espresso powder in small saucepan. Stir over low heat until coffee dissolves. Mix in vanilla. Add to cheese mixture and beat until smooth. Add eggs 1 at a time, beating just until combined. Pour filling into 2 bowls, dividing evenly. \n\nMelt chocolate in top of double broiler over simmering water, stirring until smooth. Stir chocolate into filling in bowl. Pour coffee filling into crust. Bake until partially set and crust forms on filling, about 25 minutes. Carefully spoon chocolate filling over coffee filling. Bake cheesecake until set on edges but center 2 inches still move slightly when pan is shaken, about 30 minutes. Transfer cheesecake to rack and cool 20 minutes. Maintain oven temperature.\nFor garnish: Dip spoon into melted chocolate. Wave from side to side over cheesecake, creating zigzag lines. Release and remove sides of springform pan. Spoon whipped cream into pastry bag fitted with large star tip. Pipe rosettes of cream around cake edges. Top with coffee bean candies if desired.\n\nFor topping: \nMix all ingredients in bowl. Press down on edges of cheesecake to even. Spoon topping over cheesecake. Bake 10 minutes. Transfer cheesecake to rack. Run small sharp knife around crust sides to loosen. Cool completely. Cover and chill overnight.\n\n\nFor garnish: \nDip spoon into melted chocolate. Wave from side to side over cheesecake, creating zigzag lines. Release and remove sides of springform pan. Spoon whipped cream into pastry bag fitted with large star tip. Pipe rosettes of cream around cake edges. Top with coffee bean candies if desired.\n\nOr top with melted chocolate zigzags and ganache rosettes!',
    preptime: '2 days',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjOxwkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Café con Leche and Chocolate Cheesecake',
    lastModified: '2013-01-18T18:21:01',
    notes: '',
    ingredients: [
      { qty: 0, qty2: 0, uom: '', desc: 'Crust:', isGroupHeader: true },
      {
        qty: 9,
        qty2: 0,
        uom: 'oz',
        desc: 'chocolate wafer cookies, broken',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'instant espresso powder or instant coffee powder',
        isGroupHeader: false,
      },
      {
        qty: 3,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet (not unsweetened) or semisweet chocolate, chopped',
        isGroupHeader: false,
      },
      {
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(3/4 stick) unsalted butter, melted, hot',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Filling:', isGroupHeader: true },
      {
        qty: 4,
        qty2: 0,
        uom: '',
        desc: '8-ounce packages cream cheese, room temperature',
        isGroupHeader: false,
      },
      { qty: 1.25, qty2: 0, uom: 'cups', desc: 'sugar', isGroupHeader: false },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'coffee liqueur',
        isGroupHeader: false,
      },
      {
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'instant espresso powder or instant coffee powder',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
      { qty: 4, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 6,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet (not unsweetened) or semisweet chocolate, chopped',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Topping:', isGroupHeader: true },
      {
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc: 'sour cream',
        isGroupHeader: false,
      },
      {
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'instant espresso powder or instant coffee powder',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Garnish:', isGroupHeader: true },
      {
        qty: 2,
        qty2: 0,
        uom: 'ounces',
        desc:
          'bittersweet (not unsweetened) or semisweet chocolate, chopped, melted',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Lightly sweetened whipped cream',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Coffee bean candies (optional)',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit | May 1992',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjOxwkM',
    directions:
      'For crust:\nFinely grind cookies, sugar and espresso powder in processor. Add chocolate and chop finely. Add butter and blend until moist crumbs form. Press crumbs onto bottom and up sides of 9-inch-diameter springform pan with 2 3/4-inch-high sides.\n\n\nFor filling:\nPreheat oven to 350°F. Using electric mixer, beat cream cheese and sugar in large bowl until well blended. \n\nCombine coffee liqueur and espresso powder in small saucepan. Stir over low heat until coffee dissolves. Mix in vanilla. Add to cheese mixture and beat until smooth. Add eggs 1 at a time, beating just until combined. \n\nPour filling into 2 bowls, dividing evenly. Melt chocolate in top of double broiler over simmering water, stirring until smooth. Stir chocolate into filling in bowl. Pour coffee filling into crust. Bake until partially set and crust forms on filling, about 25 minutes. \n\nCarefully spoon chocolate filling over coffee filling. Bake cheesecake until set on edges but center 2 inches still move slightly when pan is shaken, about 30 minutes. Transfer cheesecake to rack and cool 20 minutes. Maintain oven temperature.\n\n\nFor topping:\nMix all ingredients in bowl. Press down on edges of cheesecake to even. Spoon topping over cheesecake. Bake 10 minutes. Transfer cheesecake to rack. Run small sharp knife around crust sides to loosen. Cool completely. Cover and chill overnight.\n\n\nFor garnish:\nDip spoon into melted chocolate. Wave from side to side over cheesecake, creating zigzag lines. Release and remove sides of springform pan. Spoon whipped cream into pastry bag fitted with large star tip. Pipe rosettes of cream around cake edges. Top with coffee bean candies if desired.\n',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiWswgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Cheesy Bread',
    lastModified: '2012-02-03T02:44:52',
    notes: '',
    ingredients: [],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Shawn Rosa, Bellville, TX',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiWswgM',
    directions:
      'Top bread with a layer of butter, then a thin layer of sour cream.\n\nSprinkle with salt, pepper, garlic powder, shredded cheese and parsley.\n\nBroil for a few minutes.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjlxQcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Chicken Piccata',
    lastModified: '2011-07-15T02:09:23',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'chicken breasts, pounded',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'cup', desc: 'flour' },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: 'T', desc: 'oil' },
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'lemon juice',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'chicken stock',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'capers, drained and rinsed',
      },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: 'T', desc: 'butter' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy cream',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'cup', desc: 'parsley' },
    ],
    categories: '',
    yield: '4',
    equipment: '',
    source: 'Angie',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjlxQcM',
    directions:
      'Season chicken.  Dredge and fry in the oil.  Remove from pan and place on platter.\n\nAdd 2T butter, juice, capers, and stock to pan.  Scrape up brown bits and bring to boil.  Return chicken to pan to coat.\n\nReturn chicken to platter.  Add 2T butter to pan.  Add cream and parsley, thicken and stir well.  Pour over chicken.  Serve with thin pasta and broccoli and Parmesan.',
    preptime: '30 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjW8QgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Chicken Stir-Fry',
    lastModified: '2012-03-05T02:11:24',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'vegetable oil',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'sesame oil',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'pounds',
        desc: 'boneless, skinless chicken breast, cut into 1/2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'bunch broccoli, cut into florets',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'package shiitake mushrooms',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'to 3 carrots, thinly sliced',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'red bell pepper, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'onion, diced',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '(8-ounce) can sliced water chestnuts, drained',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'chicken broth',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'hoisin sauce or bean sauce',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'soy sauce',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'powdered ginger',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'cornstarch',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'Bunches buckwheat noodles',
      },
    ],
    categories: '',
    yield: 'Serves 4',
    equipment: '',
    source: 'Paula Deen',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjW8QgM',
    directions:
      '\nIn a large skillet, heat the oils over medium-high heat. Add the chicken and cook for 4 to 5 minutes or until lightly browned. Add broccoli, mushrooms, carrots, pepper, and onion and cook an additional 5 minutes, stirring frequently. Stir in the water chestnuts. In a small bowl, combine broth, hoisin sauce, soy sauce, ginger, and cornstarch. Add to chicken mixture and bring to a boil over medium-high heat. Reduce heat to medium to medium-low, and simmer for 4 to 5 minutes, or until sauce thickens. Serve over hot, cooked rice noodles.',
    preptime: '30 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjmvwkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Chicken and Vegetable Pot Pies with Cream Cheese Crust',
    lastModified: '2012-12-31T20:56:27',
    notes:
      'I add whatever veggies I have on hand, and usually bake this in a casserole dish.  It takes much longer to bake, but I like it better than individual servings.',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Crust:' },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: '(2 sticks) unsalted butter, cut into pieces, room temperature',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '8-ounce package cream cheese, cut into pieces, room temperature',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Chicken:' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '3 1/2-pound whole chicken',
      },
      { isGroupHeader: false, qty: 12, qty2: 0, uom: 'cups', desc: 'water' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'large onions, quartered',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'large carrots, coarsely chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large leek, sliced',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: '',
        desc: 'small thyme sprigs',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'bay leaf' },
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Filling and assembly:',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'olive oil',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'red bell peppers, cut into strips',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large onion, chopped',
      },
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: 'ounces',
        desc: 'shiitake mushrooms, stemmed, caps sliced',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: 'green beans, boiled 1 minute, cut into 1-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'chopped green onions',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'drained oil-packed sun-dried tomatoes, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'chopped rosemary',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: '(1 stick) unsalted butter',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'plus 2 tablespoons all purpose flour',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'milk' },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 4.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'beef base* or other concentrated beef-flavored broth*',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'egg, beaten to blend (for glaze)',
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit  | February 2001',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjmvwkM',
    directions:
      ' For crust:\nMix flour and salt in processor. Add butter and cheese; blend until moist clumps form. Shape dough into 6-inch-long log. Wrap; chill while making chicken and filling.\n\nFor chicken:\nPlace all ingredients in large pot; bring to boil. Reduce heat; simmer until chicken is cooked through, about 45 minutes. Transfer chicken to large bowl; cool. Discard skin and bones. Cut meat into 1-inch pieces; return to bowl. Strain cooking liquid into large saucepan. Boil until reduced to 3 cups stock, about 1 hour.\n\nFor filling and assembly:\nHeat 1 tablespoon oil in large skillet over medium heat. Add peppers and onion; cook until soft, about 6 minutes. Transfer to bowl with chicken. Heat 2 tablespoons oil in same skillet over medium-high heat. Add mushrooms; sauté until just brown, about 4 minutes. Add to bowl with chicken; mix in next 4 ingredients.\n\nMelt butter in large saucepan over medium heat. Add flour; whisk 2 minutes. Gradually whisk in 3 cups stock, then milk, cream and beef base; bring to boil. Season sauce with salt and pepper. Stir 3 cups sauce into chicken mixture (reserve remaining sauce for another use).\n\nCut prepared dough crosswise into 6 equal pieces. Roll out pieces to 7-inch rounds on lightly floured surface. Divide filling among six 2-cup soufflé dishes. Cover each dish with 1 dough round. Press overhang to side of dish to adhere. (Can be made 1 day ahead. Cover; chill.)\n\nPreheat oven to 350°F. Brush top of each dough round with egg glaze; cut 3 slits in each for steam to escape. Bake pies until crust is golden brown and filling is heated through, about 45 minutes.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj2kAkM: {
    status: 'active',
    subtitle:
      'Ganache-Filled Chocolate Cupcakes with Seven-Minute Meringe Frosting',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Chocolate Ganache-Filled Cupcakes',
    lastModified: '2012-03-28T01:46:17',
    notes: '',
    ingredients: [
      { qty: 0, qty2: 0, uom: '', desc: 'Cupcakes:', isGroupHeader: true },
      {
        qty: 1.75,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'unsweetened cocoa powder',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      { qty: 1.75, qty2: 0, uom: 'cups', desc: 'sugar', isGroupHeader: false },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: '(1 1/2 sticks) unsalted butter, room temperature',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'hot water', isGroupHeader: false },
      {
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc:
          'bittersweet chocolate chips (about 7 1/2 ounces; do not exceed 61% cacao)',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Filling and frosting:',
        isGroupHeader: true,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy whipping cream',
        isGroupHeader: false,
      },
      {
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'bittersweet chocolate chips (do not exceed 61% cacao)',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'sugar', isGroupHeader: false },
      {
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'large egg whites',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'cream of tartar',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Pinch of coarse kosher salt',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit, April 2011, by Jeanne Kelley',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj2kAkM',
    directions:
      'For cupcakes:\nPreheat oven to 350°F. Line two 12-cup standard muffin pans with paper liners. Sift flour, cocoa powder, baking soda, and salt into medium bowl. Using electric mixer, beat sugar and butter in large bowl until light and fluffy, about 2 minutes. Add eggs 1 at a time, beating until well incorporated after each addition. Beat in vanilla, half of flour mixture, then 1 cup hot water. Add remaining flour mixture; beat just until blended. Let batter stand until cooled and slightly thickened, about 5 minutes. Stir in chocolate chips. Divide batter among muffin papers (about 1/4 cup batter each). \n\nBake cupcakes until tester inserted into center comes out with some crumbs attached, 22 to 25 minutes. Cool cupcakes completely in pans. \n\nFor filling and frosting:\nBring cream just to boil in heavy small saucepan. Place chocolate chips in medium bowl; pour hot cream over. Let stand 1 minute, then whisk until melted and smooth. Let stand at room temperature until cool and firm, about 2 hours. (Alternatively, chill ganache until cool and firm, stirring occasionally, about 1 hour.) DO AHEAD: Can be made 4 hours ahead. Let stand at room temperature.\n\nWhisk 1/3 cup water, sugar, egg whites, cream of tartar, and coarse salt to blend in large metal bowl. Set bowl over saucepan of barely simmering water. Using electric mixer, beat mixture until soft peaks form, about 5 minutes. Remove bowl from over water and continue beating until frosting is cool to touch, stiff, and billowy, about 2 minutes. Beat vanilla into frosting. \n\nUsing thumb, press down center of each cupcake from top to bottom, forming deep pocket. Spoon or pipe ganache into pocket of each cupcake. \n\nFrost cupcakes, forming tall peaks. DO AHEAD: Cupcakes can be made 1 day ahead. Cover with cake dome and store at room temperature.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiWsAkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Chocolate Peppermint Cookies',
    lastModified: '2012-12-31T20:46:41',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Cookies:' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cups',
        desc: 'unsweetened cocoa powder',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: '(2 sticks) unsalted butter, room temperature',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'packed light brown sugar',
      },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'cup', desc: 'sugar' },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'eggs' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'vanilla extract',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chocolate chips',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Glaze:' },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: "confectioners' sugar",
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'drops mint extract, or more to taste',
      },
      {
        isGroupHeader: false,
        qty: 3.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'water',
      },
    ],
    categories: 'cookies, chocolate',
    yield: '2 dozen',
    equipment: '',
    source: 'foodnetwork.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiWsAkM',
    directions:
      ' For the cookies:\n\nPreheat oven to 350 degrees F.\n\nIn a medium bowl sift together flour, cocoa powder, baking soda and salt. Set aside. \n\nIn a mixer beat together the butter with the sugars until fluffy, about 2 minutes. Add eggs 1 at a time, beating until smooth after each addition and scraping the sides of the bowl as necessary. Add in the vanilla. \n\nOn a low speed beat in the flour mixture a little at a time. Gently mix in chocolate chips.\n\nOn an ungreased cookie sheet, drop teaspoons of the batter about 2 inches apart. Bake until just cracked on top, about 8 to10 minutes, rotating the pan halfway through the cooking time. Cool completely.\n\nFor the glaze:\n\nIn a small bowl mix together the sugar; mint extract and water until smooth. Transfer mixture to a medium resealable plastic bag and snip off the corner. Drizzle glaze over cooled cookies. Let cookies stand until set, about 10 minutes.',
    preptime: '40 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi-9gkM': {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Chorizo and Potato Spanish Tortilla Bites',
    lastModified: '2013-02-01T06:35:49',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'bunch scallions',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'extra-virgin olive oil',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'medium onions, chopped (1 1/2 cups)',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'garlic cloves, finely chopped',
      },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'black pepper',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'oz',
        desc:
          'Spanish chorizo (cured spiced pork sausage), cut into 1/4-inch dice',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'lb',
        desc:
          'yellow-fleshed potatoes such as Yukon Gold (about 4 medium), peeled and cut into 1/4-inch dice',
      },
      { isGroupHeader: false, qty: 9, qty2: 0, uom: '', desc: 'large eggs' },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'sour cream',
      },
      {
        isGroupHeader: false,
        qty: 5,
        qty2: 0,
        uom: 'oz',
        desc: 'Manchego or white Cheddar, coarsely grated (2 cups)',
      },
    ],
    categories: '',
    yield: "64 hors d'eouvres",
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi-9gkM',
    directions:
      'Put oven rack in middle position and preheat oven to 400°F. Lightly oil a 13- by 9-inch baking pan or dish.\n\nFinely chop white and green parts of scallions and reserve separately.\n\nHeat oil in a 12-inch heavy skillet over moderately high heat until hot but not smoking, then cook onions, garlic, white parts of scallions, 3/4 teaspoon salt, and 1/4 teaspoon pepper, stirring occasionally, until onions are golden, about 6 minutes. Add chorizo and cook, stirring occasionally, until just beginning to brown and release oil, about 4 minutes. Reduce heat to moderate, then stir in potatoes and cover skillet. Cook, stirring occasionally, until potatoes are tender, about 10 minutes. Transfer mixture to a large bowl and cool slightly.\n\nWhisk together eggs, sour cream, cheese, scallion greens, and remaining 1/2 teaspoon salt and 1/4 teaspoon pepper in a bowl, then pour into baking pan. Sprinkle potato mixture over eggs (some potatoes will stick out). Bake until custard is set, about 20 minutes. Cool in pan on a rack to warm or room temperature. Trim 1/2 inch off each side, then cut tortilla into 1 1/2- by 1-inch rectangles.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiokwMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Cinnamon Ice Cream',
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'whole milk',
      },
      { isGroupHeader: false, qty: 1.25, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 5,
        qty2: 0,
        uom: '',
        desc: 'cinnamon sticks',
      },
      {
        isGroupHeader: false,
        qty: 0.125,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: '',
        desc: 'large egg yolks',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
      },
    ],
    categories: 'desserts,  ice cream',
    yield: 'Makes about 7 cups',
    equipment: '',
    source: 'Bon Appétit, November 2002',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiokwMM',
    directions:
      "Combine milk, 1 cup sugar, cream, cinnamon sticks, and salt in heavy large saucepan. Bring almost to simmer over medium-high heat. Whisk egg yolks and 1/4 cup sugar in large bowl. Gradually whisk in hot milk mixture. Return custard to saucepan and stir constantly over low heat until custard thickens, about 4 minutes (do not boil). Transfer custard to large bowl set over another large bowl of ice water; let cool 30 minutes, stirring often. Cover and chill custard overnight.\n\nStrain custard. Process in ice cream maker according to manufacturer's instructions. Transfer ice cream to container; cover and freeze.",
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi44QMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Coffee Oreo Ice Cream',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'heavy cream',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'milk' },
      { isGroupHeader: false, qty: 0.667, qty2: 0, uom: 'cup', desc: 'sugar' },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'Pinch of Salt' },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'instant coffee (get decaf)',
      },
      { isGroupHeader: false, qty: 6, qty2: 0, uom: '', desc: 'egg yolks' },
      {
        isGroupHeader: false,
        qty: 10,
        qty2: 0,
        uom: '',
        desc: 'oreo cookies, crushed',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'ounce',
        desc: '(2 Tbsp) Mount Gay rum, or other golden rum',
      },
    ],
    categories: 'desserts, ice cream',
    yield: '',
    equipment: 'Ice Cream Maker',
    source: 'Simply Recipes',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi44QMM',
    directions:
      '1. Heat 1 cup of the cream, 1 cup of milk, sugar, and a pinch of salt in a medium saucepan on medium heat until steaming, but not boiling. Stir in the instant coffee until it is dissolved.\n\n2. Pour the remaining 1 cup of cream into a medium size metal bowl, set in an ice water bath in a larger bowl. Set a mesh sieve on top of the bowls. Set aside.\n\n3. In a separate bowl, whisk the egg yolks together. Slowly pour the heated milk, cream, coffee mixture into the egg yolks, whisking constantly so that the egg yolks are tempered by the warm milk, but not cooked by it. Pour the warmed egg yolks back into the saucepan.\n\n4. Stir the mixture constantly over medium heat with a heatproof, flat-bottomed spatula, scraping the bottom as you stir, until the mixture thickens and coats the spatula so that you can run your finger across the coating and have the coating not run. Be patient. This step can take 5 to 10 minutes.\n\n5. Pour the custard through the seive and stir it into the chilled cream.  Chill mixture thoroughly in the refrigerator.\n\n6. When thoroughly chilled and ready to put into your ice cream maker, add the rum and the crushed oreo cookies. Process the mixture in your ice cream maker according to the ice cream maker directions. Transfer mixture to a plastic storage container and freezer in your freezer until firm, at least 6 hours.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjp1wEM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Cream of Vegetable Soup',
    lastModified: '2010-09-28T14:53:15',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'water, vegetable or chicken stock',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'unsalted butter',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'medium onion, coarsely chopped',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'cloves garlic, minced',
      },
      {
        isGroupHeader: false,
        qty: 0.125,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg (optional)',
      },
      { isGroupHeader: false, qty: 4.5, qty2: 0, uom: 'cups', desc: 'stock' },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'white wine',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'pounds',
        desc: 'vegetable of choice, coarsely chopped',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: '(cauliflower, broccoli, carrot, spinach, etc.)',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0.5,
        uom: 'cup',
        desc: 'heavy cream or milk',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 1,
        uom: 'teaspoon',
        desc: 'ground white or black pepper',
      },
    ],
    categories: '',
    yield: 'Serves 4-6',
    equipment: '',
    source: 'The Joy of Cooking',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjp1wEM',
    directions:
      'Heat stock and butter in a large heavy saucepan or soup pot over medium-low until butter is melted.Add onion, garlic, and nutmeg, and cook, stirring occasionally until tender but not browned. Stir in stock, wine, and vegetables. Bring to a boil, reduce heat, and simmer until veg is tender, 15 – 20 minutes. Puree until smooth.\n\nReturn to the pot and stir in cream, salt, and pepper. Simmer briefly and ladle into warmed bowls. Garnish with herbs and serve with croutons or crackers.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjX_QQM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Double Chocolate Layer Cake',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'For Cake Layers',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'ounces',
        desc: 'fine-quality semisweet chocolate such as Callebaut',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'hot brewed coffee',
      },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'unsweetened cocoa powder (not Dutch process)',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'salt',
      },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: '', desc: 'large eggs' },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'vegetable oil',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'well-shaken buttermilk',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla',
      },
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'For Ganache Icing',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'pound',
        desc: 'fine-quality semisweet chocolate such as Callebaut',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy cream',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'light corn syrup',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'stick',
        desc: '(1/4 cup) unsalted butter',
      },
    ],
    categories: 'cake,  chocolate,  desserts',
    yield: '',
    equipment: 'two 10- by 2-inch round cake pans',
    source: 'Epicurious',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjX_QQM',
    directions:
      'Make cake layers: Preheat oven to 300°F. and grease pans. Line bottoms with rounds of wax paper and grease paper.\n\nFinely chop chocolate and in a bowl combine with hot coffee. Let mixture stand, stirring occasionally, until chocolate is melted and mixture is smooth.\n\nInto a large bowl sift together sugar, flour, cocoa powder, baking soda, baking powder, and salt. In another large bowl with an electric mixer beat eggs until thickened slightly and lemon colored (about 3 minutes with a standing mixer or 5 minutes with a hand-held mixer). Slowly add oil, buttermilk, vanilla, and melted chocolate mixture to eggs, beating until combined well. Add sugar mixture and beat on medium speed until just combined well. Divide batter between pans and bake in middle of oven until a tester inserted in center comes out clean, 1 hour to 1 hour and 10 minutes.\n\nCool layers completely in pans on racks. Run a thin knife around edges of pans and invert layers onto racks. Carefully remove wax paper and cool layers completely. Cake layers may be made 1 day ahead and kept, wrapped well in plastic wrap, at room temperature.\n\nMake frosting: Finely chop chocolate. In a 1 1/2- to 2-quart saucepan bring cream, sugar, and corn syrup to a boil over moderately low heat, whisking until sugar is dissolved. Remove pan from heat and add chocolate, whisking until chocolate is melted. Cut butter into pieces and add to frosting, whisking until smooth.\n\nTransfer frosting to a bowl and cool, stirring occasionally, until spreadable (depending on chocolate used, it may be necessary to chill frosting to spreadable consistency).\n\nSpread frosting between cake layers and over top and sides. Cake keeps, covered and chilled, 3 days. Bring cake to room temperature before serving.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjP3AIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Double Chocolate Mocha Brownies',
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'sticks',
        desc: 'unsalted butter',
      },
      {
        isGroupHeader: false,
        qty: 5,
        qty2: 0,
        uom: 'oz.',
        desc: 'chopped unsweetened (we use 60 - 90%) chocolate',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'c.', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'T',
        desc: 'espresso powder (I use decaf or reduce the amount)',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp.', desc: 'vanilla' },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'tsp.', desc: 'salt' },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'lg. eggs' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'c.',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'c.',
        desc: 'semisweet chocolate chips',
      },
    ],
    categories: 'desserts, chocolate',
    yield: '',
    equipment: '',
    source: 'Gourmet, December 2002',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjP3AIM',
    directions:
      '1.  Preheat oven to 375°F.  Line a 15x10x1" pan with foil, leaving a 2" overhang (we usually use a smaller pan to make thicker brownies).  Grease well with 1T butter.\n\n2.  Melt remaining 11T\'s butter with unsweetened chocolate over a double boiler or in a microwave.  Remove from heat and whisk in sugar, espresso, vanilla, and salt (mixture will be grainy); then add eggs one at a time, whisking until smooth after each addition.\n\n3.  Toss together flour and chocolate chips in a separate bowl.  Add to batter, stirring until just combined.\n\n4.  Spread batter evenly in baking pan and bake in middle of oven until top is firm and edges begin to pull away from sides of pan, about 20 minutes (DO NOT OVER BAKE!).\n\n5.  Cool in pan on rack for 5 minutes, then carefully lift brownies from pan by lifting foil.  cool on rack 10 minutes.  Cut into squares and lift with spatula.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOngIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Espresso Chocolate Cheesecake Harris',
    lastModified: '2010-09-28T14:53:15',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'For crust' },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'finely ground chocolate wafers (about 26)',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'stick',
        desc: '(6 tablespoons) unsalted butter, melted',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'For filling' },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'pounds',
        desc: 'cream cheese, softened',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'cornstarch',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'plus 1 teaspoon vanilla extract',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sour cream' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'instant espresso powder',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: 'ounces',
        desc:
          'fine-quality bittersweet chocolate (not unsweetened), melted and cooled',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'chilled heavy cream, whipped into soft peaks',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Garnish: bittersweet and milk chocolate curls',
      },
    ],
    categories: 'desserts, cheesecakes',
    yield: '',
    equipment: '',
    source: 'Gourmet, January 1997',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOngIM',
    directions:
      'Make crust:\nPreheat oven to 325°F. and butter a 9-inch springform pan.\n\nIn a bowl stir together wafer crumbs and butter. Press mixture onto bottom and 1 inch up side of pan. Bake crust in middle of oven 10 minutes and cool in pan on a rack.\n\nWhen crust is cool, wrap pan in foil by putting in middle of am 18-inch square of heavy-duty foil and crimping foil around bottom and up side of pan to secure.\n\nMake filling:\nIn a large bowl with an electric mixer beat eggs with sugar until light and fluffy. Add cream cheese and beat until smooth. Sift cornstarch into mixture and add 1 teaspoon vanilla and sour cream. Beat filling until smooth and transfer 2 cups to a small bowl.\n\nIn a small cup stir together espresso powder and remaining tablespoon vanilla, stirring until espresso powder is dissolved. Stir espresso mixture into filling in small bowl.\n\nAdd chocolate to filling in large bowl, stirring until combined well, and fold in whipped cream. Pour chocolate filling into crust. Spoon espresso filling evenly over chocolate filling and spread to form an even layer, being careful not to mix layers.\n\nPut springform pan in a larger shallow baking pan and pour enough hot water to reach halfway up side of springform pan. Bake cheesecake in middle of oven 1 1/2 hours. Turn off oven and let cheesecake stand 15 minutes with oven door closed. Transfer cheesecake in pan to a rack and cool completely. Chill cheesecake, covered, at least 1 day and up to 2. Run a thin knife around edge of pan and remove side.\n\nGarnish cheesecake with chocolate curls. To serve, cut cheesecake with a sharp knife dipped in hot water.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi_iwMM: {
    status: 'active',
    subtitle: 'Triple Chocolate Mousse Cake',
    canEmailOwner: true,
    isFavorite: true,
    title: "Gabriel's 2nd Birthday Cake",
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Cake Ingredients',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'ounces',
        desc: 'fine-quality semisweet chocolate',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cups',
        desc: 'hot brewed coffee',
      },
      { isGroupHeader: false, qty: 1.5, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cups',
        desc: 'unsweetened cocoa powder (not Dutch process)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: '',
        desc: 'scant teaspoon baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.667,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'large eggs' },
      {
        isGroupHeader: false,
        qty: 0.375,
        qty2: 0,
        uom: 'cup',
        desc: 'vegetable oil',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cups',
        desc: 'well-shaken buttermilk',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla',
      },
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Mousse Ingredients',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'Vegetable oil' },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet chocolate, chopped',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: 'milk chocolate, chopped',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: 'good-quality white chocolate',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'cold water',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'unflavored gelatin',
      },
      {
        isGroupHeader: false,
        qty: 5,
        qty2: 0,
        uom: '',
        desc: 'large egg yolks',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'half and half',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chilled whipping cream',
      },
    ],
    categories: 'cake, chocolate, desserts',
    yield: '',
    equipment: '',
    source: "Epicurious/Angie's Edit",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi_iwMM',
    directions:
      'Make cake layers:\n\nPreheat oven to 300°F. and grease pans. Line bottoms with rounds of wax paper and grease paper.\n\nFinely chop chocolate and in a bowl combine with hot coffee. Let mixture stand, stirring occasionally, until chocolate is melted and mixture is smooth.\n\nInto a large bowl sift together sugar, flour, cocoa powder, baking soda, baking powder, and salt. In another large bowl with an electric mixer beat eggs until thickened slightly and lemon colored (about 3 minutes with a standing mixer or 5 minutes with a hand-held mixer). Slowly add oil, buttermilk, vanilla, and melted chocolate mixture to eggs, beating until combined well. Add sugar mixture and beat on medium speed until just combined well.  Pour batter into pan and bake in middle of oven until a tester inserted in center comes out clean, 1 hour to 1 hour and 10 minutes.\n\nCool layers completely in pans on racks. Run a thin knife around edges of pans and invert onto rack.  Cake layers may be made 1 day ahead and kept, wrapped well in plastic wrap, at room temperature.\n\nMake mousse layers:\n\nBrush 9-inch-diameter springform pan with oil. Place each chocolate in separate medium bowl. Combine 1/4 cup water and gelatin in small bowl. Let stand until gelatin softens, about 10 minutes.\n\nMeanwhile, beat yolks and sugar in large bowl until mixture is pale yellow and very thick, about 5 minutes. Bring half and half just to simmer in heavy large saucepan. Gradually whisk hot half and half into beaten egg mixture. Return mixture to same saucepan. Stir over medium heat until custard thickens and leaves path on back of spoon when finger is drawn across, about 3 minutes (do not boil). Remove custard from heat. Add softened gelatin; stir until gelatin dissolves. Strain mixture into large glass measuring cup. Immediately pour 1/3 of hot custard over each. Stir each chocolate with separate spoon until melted and smooth. If mixture cools before chocolate is completely melted, set bowl over saucepan of simmering water and stir just until chocolate melts. Cool chocolate mixtures to room temperature, stirring occasionally.\n\nBeat cream in large bowl until stiff peaks form. Divide whipped cream equally among bowls of chocolate, using about 1 1/3 cups cream for each. Fold whipped cream into chocolate in each bowl.\n\nAssemble:\n\nCarefully slice cake into two layers.  Return bottom layer into springform (make sure sides of pan are oiled)  Pour bittersweet chocolate mixture onto bottom layer of chocolate cake in springform pan. Smooth top with spatula. Freeze until firm, about 15 minutes. Pour milk chocolate mixture over bittersweet chocolate layer. Smooth top with spatula. Freeze until firm, about 15 minutes. Pour white chocolate mixture over milk chocolate mixture. Smooth top with spatula. Freeze until firm, about 15 minutes.  May serve as is, or top with other half of cake and cover with ganache.\n\n(Torte can be prepared 2 days ahead. Cover with plastic and store in refrigerate.)\n\nRun sharp knife around sides of pan. Remove pan sides. Cut torte into wedges and serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjQ1gQM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Garden Vegetable and Chicken Enchiladas',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(1/2 stick) butter, divided',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 3,
        uom: 'cups',
        desc: 'coarsely chopped zucchini and/or yellow crookneck squash',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'chopped onion',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'fresh corn kernels or frozen, thawed',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '4-ounce can diced mild green chilies',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'chopped fresh cilantro',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'chicken, roasted',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'chili powder',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'ground cumin',
      },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'cups',
        desc: 'whole milk',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc:
          '(packed) grated Monterey Jack and/or sharp cheddar cheese (about 8 ounces)',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: '',
        desc: '8-inch warm flour tortillas',
      },
    ],
    categories: 'mexican, mains, chicken, vegetables',
    yield: '',
    equipment: '',
    source: "epicurious.com - angie's edited version",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjQ1gQM',
    directions:
      'Preheat oven to 350°F. Melt 1 tablespoon butter in heavy large skillet over medium-high heat. Add zucchini and onion and sauté until vegetables are just tender, about 4 minutes. Mix in 1 cup corn, canned chilies, chicken, and 1/4 cup cilantro; season filling with salt and pepper.\n\nMelt 3 tablespoons butter in heavy medium saucepan over medium-high heat. Whisk in chili powder, flour and cumin; stir 30 seconds. Gradually whisk in milk. Cook until sauce is thick and bubbling, whisking occasionally, about 5 minutes. Add 1 1/2 cups cheese; whisk until smooth. Season with salt and pepper.\n\nSpread 1/4 cup sauce in bottom of 13 x 9 x 2-inch glass baking dish. Mix 3/4 cup sauce into filling. Place generous 1/3 cup filling in center of 1 tortilla; roll up to enclose filling. Place enchilada in baking dish. Repeat with remaining tortillas and filling. Cover enchiladas with remaining sauce, then sprinkle with remaining 1/2 cup corn and 1/2 cup cheese.\n\nBake vegetable enchiladas until heated through, about 45 minutes. Sprinkle with 1/4 cup cilantro and serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj-uggM': {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Green Chile-Chicken Casserole',
    lastModified: '2012-02-18T00:10:38',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'C',
        desc: 'low sodium chicken broth',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'C',
        desc: 'chopped roasted Hatch green chiles',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'C',
        desc: 'sour cream',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'c', desc: 'cream' },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'ground cumin',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'freshly ground black pepper',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp.', desc: 'boullion' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'garlic clove, minced',
      },
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: '',
        desc: '(6-inch) corn or flour tortillas',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'lbs',
        desc: 'shredded cooked chicken breast',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'C',
        desc: 'shredded cheese',
      },
    ],
    categories: '',
    yield: 'Serves 4',
    equipment: '',
    source: 'SweetteainTexas.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj-uggM',
    directions:
      '\nPreheat oven to 350°.\n \nCombine the first 9 ingredients in a large saucepan. Whisk together. Bring to a boil, stirring constantly. Remove from heat.\n \nSpread 1/2 of the soup mixture in a 8×8 -inch baking dish coated with cooking spray. Arrange 4 tortillas Green Chile-Chicken Casseroleover the soup mixture. Spread 1/2 of the chicken over tortillas. Top with 1/2 of the cheese.\n \nRepeat layer once more, ending with the cheese. Bake for 30 minutes or until bubbly.',
    preptime: '45 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjmwggM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Green Chile-Chicken Casserole',
    lastModified: '2012-02-18T00:10:40',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'C',
        desc: 'low sodium chicken broth',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'C',
        desc: 'chopped roasted Hatch green chiles',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'C',
        desc: 'sour cream',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'c', desc: 'cream' },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'ground cumin',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'freshly ground black pepper',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp.', desc: 'boullion' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'garlic clove, minced',
      },
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: '',
        desc: '(6-inch) corn or flour tortillas',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'lbs',
        desc: 'shredded cooked chicken breast',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'C',
        desc: 'shredded cheese',
      },
    ],
    categories: '',
    yield: 'Serves 4',
    equipment: '',
    source: 'SweetteainTexas.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjmwggM',
    directions:
      '\nPreheat oven to 350°.\n \nCombine the first 9 ingredients in a large saucepan. Whisk together. Bring to a boil, stirring constantly. Remove from heat.\n \nSpread 1/2 of the soup mixture in a 8×8 -inch baking dish coated with cooking spray. Arrange 4 tortillas Green Chile-Chicken Casseroleover the soup mixture. Spread 1/2 of the chicken over tortillas. Top with 1/2 of the cheese.\n \nRepeat layer once more, ending with the cheese. Bake for 30 minutes or until bubbly.',
    preptime: '45 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiF5QcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Hamburger Buns',
    lastModified: '2011-12-09T15:19:25',
    notes: 'Ready in 1 hr 30 min.\n',
    ingredients: [
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'milk' },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'water' },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'cup', desc: 'butter' },
      {
        isGroupHeader: false,
        qty: 4.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '(.25 ounce) package instant yeast',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'white sugar',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'salt',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'egg' },
    ],
    categories: '',
    yield: '1 dozen',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiF5QcM',
    directions:
      'In a small saucepan, heat milk, water and butter until very warm, 120 degrees F (50 degrees C).\n\nIn a large bowl, mix together 1 3/4 cup flour, yeast, sugar and salt. Mix milk mixture into flour mixture, and then mix in egg. Stir in the remaining flour, 1/2 cup at a time, beating well after each addition. When the dough has pulled together, turn it out onto a lightly floured surface, and knead until smooth and elastic, about 8 minutes.\n\nDivide dough into 12 equal pieces. Shape into smooth balls, and place on a greased baking sheet. Flatten slightly. Cover, and let rise for 30 to 35 minutes.\nBake at 400 degrees F (200 degrees C) for 10 to 12 minutes, or until golden brown.\n\nFor Hot Dog Buns: Shape each piece into a 6x4 inch rectangle. Starting with the longer side, roll up tightly, and pinch edges and ends to seal. Let rise about 20 to 25 minutes. Bake as above. These buns are pretty big. I usually make 16 instead of 12.',
    preptime: '20 Min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOjAgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Hatch Skillet Mac and Cheese',
    lastModified: '2011-12-06T04:08:44',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'lb',
        desc: 'elbow macaroni',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'tbsp', desc: 'butter' },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'chicken broth',
      },
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: '',
        desc: 'tomatillos, husked and chopped',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: '',
        desc: 'Hatch chiles, peeled, seeded and chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'medium white onion, chopped',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'cloves garlic, minced',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'cilantro leaves, loosely packed',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tbsp',
        desc: 'vegetable oil',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'half and half',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'Monterey Jack cheese, shredded',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'white cheddar cheese, shredded',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'oz',
        desc: 'cream cheese, cubed',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chicken breast, cooked and shredded',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'tortilla chips, crushed',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tbsp',
        desc: 'butter, melted',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'Monterey Jack cheese, grated',
      },
    ],
    categories: 'Pasta, cheese, chiles',
    yield: '6-8',
    equipment: 'Skillet!',
    source: 'Central Market',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOjAgM',
    directions:
      'Cook pasta until al dente and drain. Butter 12-inch cast iron skillet. \n\nHeat broth with tomatillos and Hatch chiles and cook for 10 minutes. Transfer to food processor and pulse. Add onion, garlic, cilantro and salt. \n\nHeat oil in saucepan. Add tomatillo mixture and cook over medium-low heat for six minutes. Stir in half and half and cheeses until heated through. Toss in pasta and chicken and place in cast iron skillet. \n\nMix last four ingredients for topping and spread over pasta mixture. Place skillet under broiler for 15 minutes until golden brown.',
    preptime: '45 minutes',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjLsgMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Hazelnut Praline Cheesecake',
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Crust' },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc: 'cake flour',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc:
          'plus 2 tablespoons (1 1/4 sticks) chilled unsalted butter, cut into 1/2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: '(packed) golden brown sugar',
      },
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Hazelnut Praline Filling',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: '8-ounce packages cream cheese, room temperature',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
      },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
    ],
    categories: 'desserts, cheescakes',
    yield: '',
    equipment: '',
    source: 'Bon Appétit, February 1998',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjLsgMM',
    directions:
      'For crust:\n\nPosition rack in center of oven; preheat to 350°F. Butter 9-inch-diameter springform pan with 2 3/4-inch-high sides. Mix flour, butter and brown sugar in processor until moist clumps form. Press mixture onto bottom of prepared pan. Bake until crust is golden, about 25 minutes. Cool. Wrap outside of pan with double layer of heavy-duty foil.\n\nFor filling:\n\nReserve 1/2 cup praline for garnish. Grind remaining praline to powder in processor. Beat cream cheese and su gar in large bowl until smooth. Beat in flour, then cream and vanilla. Beat in eggs 1 at a time. Fold in praline powder.\n\nTransfer filling to crust. Place springform pan in roasting pan. Pour enough hot water into roasting pan to come halfway up sides of springform pan. Bake until center is just set, about 1 hour 20 minutes.\n\nTransfer cake to rack. Run small sharp knife around sides of cake to loosen. Cool completely. Remove foil from pan sides. Chill cake overnight. (Can be prepared 3 days ahead. Cover and keep refrigerated.)\n\nRelease pan sides. Transfer cake to platter. Mound reserved coarsely crushed praline atop center of cake and serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiBygMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Holiday Sugar Cookies',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'c',
        desc: 'unsalted butter, soft',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'c', desc: 'brown sugar' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'large egg' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'Mexican vanilla',
      },
      { isGroupHeader: false, qty: 2.667, qty2: 0, uom: 'c', desc: 'flour' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'baking powder',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'nutmeg (opt.)',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'cinnamon (opt.)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'vanilla bean (opt.)',
      },
    ],
    categories: 'desserts',
    yield: '~36 cookies',
    equipment: '',
    source: 'Epicurious.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiBygMM',
    directions:
      'Cream butter & sugar.  Add egg and vanilla.  Sift in remaining ingredients and mix.  Divide, chill ~3 hours, roll dough 1/4" and cut out cookies.  Bake at 350° for 11 minutes.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjX8QgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Honey Oat Bread',
    lastModified: '2012-03-05T02:18:34',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'warm water (110-115 degrees)',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'teaspoon', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'yeast',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'egg' },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'cup', desc: 'honey' },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'cup', desc: 'butter' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'Salt' },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'whole wheat or bread flour',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'oats' },
    ],
    categories: '',
    yield: '1 loaf',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjX8QgM',
    directions:
      '\n\n\n1\nMix water with yeast and sugar and let sit until foamy (about 10 minutes). In the meanwhile, put the 3 eggs (in the shell) in a bowl of hot water (to bring the eggs to room temperature).\n2\nPut flour, salt, and gluten in bowl.\n3\nPour in yeast mixture and stir until combined. Add in honey, eggs, and crisco until combined, scraping sides. (I usually use an egg beater for this).\n4\nAdd more wheat flour and knead until smooth.\n5\nLet set in warm place for about an hour until double in size.\n6\nDivide into 3 loaves. Let rise again in warm place until doubled (about 1-1/2 hours).\n7\nBake as regular bread (I bake about 35 minutes at 375 or until the interior of the bread is 190 degrees using electric thermometer if you have one).\n8\nEnjoy!',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjW7gkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: "Lela's Chicken Noodle Soup",
    lastModified: '2013-01-31T16:13:54',
    notes: '',
    ingredients: [],
    categories: '',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjW7gkM',
    directions:
      "Angie-\nI am not good at the measurement part of the salt and pepper, but essentially those are the only 2 seasonings I use.  It's a very simple recipe, it just takes a little bit of  process time.\nI made you essentially my dad's recipe for chicken and dumplins, and then added some carrots and used my handy dandy pasta cutter to make the noodles a little smaller than I generally do the dumplins.\nIf you have a pre-made broth that you like  you can skip straight to the dumplins but I prefer making my own broth since I need the cooked chicken anyway.\n\nI usually buy a large pack of chicken legs and thighs--I used to cut up a whole chicken, but I realized that this system works as well and is much less time consuming.\nCook chicken with rough cut celery and rough cut onion in a stockpot of water covering the chicken until chicken is cooked and falling off the bone.\nI usually do this the night before so I can skim off the fat once it cools.\nStrain the broth and stick it back in a stockpot.\nSave the chicken for later and discard the rest.\n\nUse 1 egg and 1 cup of warm broth and add enough self-rising flour to make dough that's able to be rolled out on a floured board. You can use regular flour, but I like the little bit of fluffiness that the SR flour makes.\nRoll out to about 1/4\" thickness and cut to desired shape.  For chicken noodle soup, I used our pasta maker on fettuccine, but when I make dumplins, I usually cut them about 1/2-3/4\" wide and 1\" long and cut them diamond shaped.\nHeat the remainder of the broth (add water to desired consistency and salt and pepper to taste) to a pretty good boil.  I tend to leave the broth a little thicker for the dumplins to make it heartier and thin the broth a little more for a soup.  If you want to add veggies, add them here, along with chunks of the chicken that was cooked.\nDrop noodles into the remaining broth and cook them until they no longer float (usually 7-10 minutes).  Lower heat and simmer until ready to eat.\n\nI think that's everything.\nHope they are tasty.\n___________________________________________________________\nLet all your speech be seasoned with grace.  Colossians 4:6",
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjemAkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Lemon Raspberry-Filled Cupcakes',
    lastModified: '2012-03-28T01:49:18',
    notes: '',
    ingredients: [
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: '(12 tablespoons) unsalted butter, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'powdered sugar, divided',
        isGroupHeader: false,
      },
      {
        qty: 4.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'finely grated lemon peel, divided',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc: 'self-rising flour',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'buttermilk',
        isGroupHeader: false,
      },
      {
        qty: 4,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'fresh lemon juice, divided',
        isGroupHeader: false,
      },
      {
        qty: 12,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'plus 1 tablespoon seedless raspberry jam',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Fresh raspberries (for garnish)',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit, April 2008,  Janet Taylor McCracken',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjemAkM',
    directions:
      'Preheat oven to 350°F. Line 12 muffin cups with paper liners. Using electric mixer, beat butter, 1 1/2 cups powdered sugar, and 3 teaspoons lemon peel in large bowl until blended, then beat until fluffy and pale yellow. Add eggs 1 at a time, beating to blend after each addition. Beat in half of flour. Add buttermilk and 2 tablespoons lemon juice; beat to blend. Beat in remaining flour.\n\nDrop 1 rounded tablespoonful batter into each muffin liner. Spoon 1 teaspoon raspberry jam over. Cover with remaining batter, dividing equally.\n\nBake cupcakes until tester inserted halfway into centers comes out clean, about 23 minutes. Cool cupcakes in pan on rack. Meanwhile, whisk remaining 1 1/2 cups powdered sugar, 2 tablespoons lemon juice, and 1 1/2 teaspoons lemon peel in small bowl. Spoon half of icing over 6 cupcakes. Whisk 1 tablespoon raspberry jam into remaining icing. Spoon over remaining cupcakes. Let stand until icing sets, about 30 minutes. Garnish with raspberries.\n\nI like it best with lemon cream cheese frosting and a raspberry on top!',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhipkwMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Malted Milk Chocolate Cheesecake',
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'unsalted butter, softened',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Cookie crust' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc:
          'One 10-ounce package butter biscuits, wafers, or shortbread cookies',
      },
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'walnuts',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'granulated sugar',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: '(8 tablespoons, 1 stick) unsalted butter, melted and cooled',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Cheesecake' },
      {
        isGroupHeader: false,
        qty: 14,
        qty2: 0,
        uom: 'ounces',
        desc: 'milk chocolate, finely chopped',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'malted milk powder',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'pounds',
        desc: 'cream cheese, softened',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'granulated sugar',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'extra-large eggs, at room temperature',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sour cream' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'pure vanilla extract',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'quart',
        desc: 'boiling water',
      },
    ],
    categories: 'desserts, cheesecakes',
    yield: 'Makes one 9 1/2-inch round cake, 12 to 14 servings',
    equipment:
      '9 1/2-inch round springform pan, heavy-duty aluminum foil, food processor or rolling pin, double boiler, small saucepan, whisk, electric stand mixer with flat beater attachment, or large mixing bowl and handheld mixer, two rubber spatulas, fine-mesh strainer, 12-inch round cake pan or large roasting pan, cooling rack',
    source:
      'Epicurious, November 2007 (from The Essential Baker by Carole Bloom)',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhipkwMM',
    directions:
      "Center a rack in the oven and preheat it to 300°F.\n\nUsing a paper towel or your fingertips, generously butter the inside of the springform pan. Use a double layer of heavy-duty aluminum foil to wrap tightly around the bottom of the pan. This prevents any water from the water bath seeping into the pan.\n\nCookie crust\nIn the work bowl of a food processor fitted with the steel blade, combine the cookies, walnuts, and sugar and pulse until the cookies are finely ground, about 2 minutes. Or place the cookies in a sturdy plastic bag and seal it. Use a rolling pin to crush the cookies to a very fine crumb consistency. Finely chop the walnuts and add them with the sugar to the cookies, seal the bag, and shake to blend together evenly.\n\nTransfer the cookie crumb mixture to a medium bowl and add the butter. Use a rubber spatula or a spoon to toss the mixture together to moisten all of the cookie crumbs.\n\nTransfer the crust to the prepared springform pan and use your fingertips to press it into the bottom and partway up the sides of the pan. Chill the crust while preparing the cheesecake batter.\n\nCheesecake\nPlace the milk chocolate in the top of a double boiler over low heat. Stir often with a rubber spatula to help melt evenly. Remove the top pan of the double boiler and wipe the bottom and sides very dry.\n\nOr place the milk chocolate in a microwave-safe bowl and melt on low power for 30-second bursts. Stir with a rubber spatula after each burst.\n\nPlace the cream in a small saucepan and bring to a boil over medium heat. Turn off the heat, add the malted milk powder, and whisk to distribute evenly. Cover the pan and let the mixture steep for 10 minutes, then strain the cream.\n\nPlace the cream cheese in the bowl of an electric stand mixer or in a large bowl. Use the flat beater attachment or a hand-held mixer to beat the cream cheese on medium speed until it's fluffy, about 2 minutes.\n\nAdd the sugar and cream together very well. Stop occasionally and scrape down the sides and bottom of the bowl with a rubber spatula.\n\nOne at a time, add the eggs to the cream cheese mixture, beating well after each addition. At first the eggs will sit on top of the cream cheese mixture, but stop often to scrape down the sides and bottom of the mixing bowl with a rubber spatula. This will help the mixture to blend. The mixture may also look curdled as the eggs are added, but as you stop and scrape down the bowl, the mixture will smooth out.\n\nAdd the sour cream and vanilla to the cream cheese mixture and stir together to combine.\n\nAdd the strained cream to the batter and blend thoroughly, then add the melted milk chocolate and mix together.\n\nTransfer the batter into the crust in the springform pan. Use a rubber spatula to smooth and even the top.\n\nPlace the springform pan in the larger cake pan or roasting pan and set the pan on the oven rack. Carefully pour the boiling water into the bottom pan until it reaches halfway up the side of the springform pan. Baking the cake in a water bath cushions it from the heat and adds extra moisture to the oven, which keeps the top of the cake from cracking.\n\nBake the cake for 1 hour and 45 minutes, until the top is set, but jiggles slightly. Remove the pan from the oven and transfer the cheesecake to a rack.\n\nRemove the foil and let the cheesecake cool completely. Cover the top of the cheesecake with waxed paper and wrap the pan tightly with aluminum foil. Refrigerate the cake for at least 6 hours before serving. To unmold the cheesecake, dip a thin-bladed knife in hot water and dry, then run it around the inner edge of the pan. Release the clip on the rim of the pan and gently lift it off the cake.\n\nKeeping\nStore the cheesecake, covered with waxed paper and tightly wrapped with aluminum foil, in the refrigerator up to 4 days. To freeze up to 2 months, wrap the cake pan tightly in several layers of plastic wrap and aluminum foil. Use a large piece of masking tape and an indelible marker to label and date the contents. If frozen, defrost overnight in the refrigerator and bring to room temperature at least 30 minutes before serving.\n\nMaking a change\nAdd 1 1/2 cups finely chopped walnuts to the batter before turning it into the pan.\n\nAdding style\nDrizzle the top of the cooled and chilled cheesecake with fine lines or designs of milk chocolate.\n\nServe each slice of cheesecake with a dollop of lightly sweetened whipped cream and scatter about 1 teaspoon of shaved milk chocolate over the cream.",
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj_twkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Maple-Pecan Scones',
    lastModified: '2012-12-31T20:58:14',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(3/4 stick) chilled unsalted butter, cut into pieces',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'toasted chopped pecans',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'large egg yolks',
      },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'maple extract',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'egg, beaten to blend with 1 teaspoon water (for glaze)',
      },
    ],
    categories: '',
    yield: '12',
    equipment: '',
    source: ' Bon Appétit  | October 2002  |  Piece of Cake, New Orleans, LA',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj_twkM',
    directions:
      ' Preheat oven to 425°F. Line large baking sheet with parchment paper. Mix flour, sugar, baking powder, and salt in large bowl. Using fingertips, rub in butter until mixture resembles coarse meal. Stir in chopped pecans.\n\nWhisk cream, egg yolks, and maple extract in medium bowl to blend. Pour over flour mixture; stir until moist clumps form. Turn dough out onto lightly floured work surface and knead gently until smooth, about 4 turns. Pat dough out to 1/2-inch-thick round. Using 2-inch-diameter fluted cutter, cut out scones. Gather scraps. Pat out dough and cut out additional scones. Transfer scones to prepared baking sheet. Brush with egg glaze.\n\nBake until scones are browned and tester inserted into center comes out clean, about 16 minutes. Cool slightly. Serve warm or at room temperature.\n\n',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiI8QMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Marshmallows',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'powdered sugar',
      },
      {
        isGroupHeader: false,
        qty: 3.5,
        qty2: 0,
        uom: '',
        desc: 'envelopes / 2 tbsp + 2 1/2 tsp gelatin',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'cold water',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'lt. corn syrup',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'hot water (~115°)',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'large egg whites',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'vanilla' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'colors or flavors',
      },
    ],
    categories: 'desserts',
    yield: '~96',
    equipment: '',
    source: 'Epicurious.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiI8QMM',
    directions:
      'Oil bottom and sides of 13x9x2" pan and dust w/ powdered sugar.\n\nIn large bowl, sprinkle gelatin over cold water.  Soften.\n\nIn 3 qt. saucepan, cook sugar, syrup, hot water & salt over low heat, stirring with wooden spoon until sugar dissolves.  Increase heat to moderate and boil (do not stir) to 240°, ~12 minutes.\n\nPour mix over gelatin.  Stir to dissolve.  Beat on high until tripled, ~6-10 min.\n\nBeat whites to stiff peak.  Add eggs and vanilla to syrup mixture.  Beat until just combined.\n\nPour into pan.  sift 1/4 cup powdered sugar over.  Chill 3 hours to 1 day.\n\nFlip out, cut and toss in powdered sugar.  Will keep 1 week.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOlQUM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: "Martha's Cornbread",
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'c',
        desc:
          'vegetable shortening, plus more for pan (I prefer softened butter)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'unsalted butter',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'corn kernels',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'all-purpose flour',
      },
      { isGroupHeader: false, qty: 0.333, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'stone-ground yellow cornmeal',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'milk' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large egg, lightly beaten',
      },
    ],
    categories: 'sides',
    yield: 'Makes one 8" square',
    equipment: '',
    source: "Martha's Baking Handbook",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOlQUM',
    directions:
      'Preheat the oven to 425°F. Coat an 8" square baking pan with vegetable shortening; set aside.\n\nMelt butter in a medium saute pan over med-high heat; add corn. Cook, stirring occasionally, until corn has softened and some of the kernels have begun to turn light golden brown, about 5 minutes. Remove from heat and set aside to cool slightly.\n\nIn a medium bowl, whisk together flour, sugar, baking powder, and salt. Stir in the cornmeal. Using a pastry blender, cut shortening into mixture until it resembles coarse meal. Add milk, egg, and reserved corn; stir to combine. Do not overmix.\n\nTransfer butter to the prepared pan, and bake until the top begins to turn golden and a cake tester inserted in the center comes out clean, 18 to 20 minutes. Transfer to a wire rack to cool completely. Cornbread is best served the day it is baked, but it can be kept at room temperature, wrapped well in plastic, for up to one day.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjemwgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: "Martha's Seven Minute Frosting",
    lastModified: '2012-01-29T02:34:49',
    notes: '',
    ingredients: [
      {
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'plus 2 tablespoons sugar',
        isGroupHeader: false,
      },
      { qty: 0.667, qty2: 0, uom: 'cup', desc: 'water', isGroupHeader: false },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'light corn syrup',
        isGroupHeader: false,
      },
      {
        qty: 6,
        qty2: 0,
        uom: '',
        desc: 'large egg whites, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'For Coconut Variation:',
        isGroupHeader: true,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc:
          'Add 1/2 teaspoon pure coconut extract at the end of step 3, whisking to combine.',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'For Coffee Variation:',
        isGroupHeader: true,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc:
          'Add 2 tablespoons pure coffee extract at the end of step 3, whisking to combine.',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjemwgM',
    directions:
      'Combine 1 1/2 cups sugar with the water and corn syrup in a small saucepan; clip a candy thermometer to side of pan. Bring to a boil over medium heat, stirring occasionally, until sugar dissolves. Continue boiling, without stirring, until syrup reaches 230 degrees.\nMeanwhile, in the bowl of a standing electric mixer fitted with the whisk attachment, whisk egg whites on medium-high speed until soft peaks form. With mixer running, add remaining 2 tablespoons sugar, beating to combine.\nAs soon as sugar syrup reaches 230 degrees, remove from heat. With mixer on medium-low speed, pour syrup down side of bowl in a slow, steady stream. Raise speed to medium-high; whisk until mixture is completely cool (test by touching the bottom of the bowl) and stiff (but not dry) peaks form, about 7 minutes. Use immediately.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjGowgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: "Martha's Snickerdoodle Cupcakes",
    lastModified: '2012-01-29T02:36:53',
    notes: '',
    ingredients: [
      {
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'cake flour (not self- rising), sifted',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'ground cinnamon, plus 1/2 teaspoon for dusting',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: '(2 sticks) unsalted butter, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 1.75,
        qty2: 0,
        uom: 'cups',
        desc: 'sugar, plus 2 tablespoons for dusting',
        isGroupHeader: false,
      },
      {
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'large eggs, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'pure vanilla extract',
        isGroupHeader: false,
      },
      { qty: 1.25, qty2: 0, uom: 'cups', desc: 'milk', isGroupHeader: false },
    ],
    categories: '',
    yield: 'Makes 28',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjGowgM',
    directions:
      'Preheat oven to 350 degrees. Line standard muffin tins with paper liners. Sift together both flours, baking powder, salt, and 1 tablespoon cinnamon.\nWith an electric mixer on medium-high speed, cream butter and sugar until pale and fluffy. Add eggs, one at a time, beating until each is incorporated, scraping down sides of bowl as needed. Beat in vanilla. Reduce speed to low. Add flour mixture in three batches, alternating with two additions of milk, and beating until combined after each.\nDivide batter evenly among lined cups, filling each three-quarters full. Bake, rotating tins halfway through, until a cake tester inserted in centers comes out clean, about 20 minutes. Transfer tins to wire racks to cool completely before removing cupcakes. Cupcakes can be stored up to 2 days at room temperature, or frozen up to 2 months, in airtight containers.\nTo finish, combine remaining 1/2 teaspoon cinnamon and 2 tablespoons sugar. Using a pastry bag fitted with a large plain tip (Ateco No. 809 or Wilton No. 1A), pipe frosting on each cupcake: Hold bag over cupcake with tip just above top, and squeeze to create a dome of frosting, then release pressure and pull up to form a peak. Using a small, fine sieve, dust peaks with cinnamon-sugar. Cupcakes are best eaten the day they are frosted; keep at room temperature until ready to serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi35AIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: "Mary's Sugar Cookies",
    lastModified: '2010-09-28T14:53:16',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'cream of tartar',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'sticks',
        desc: '(1 cup) unsalted butter, softened',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'confectioners sugar',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'large egg' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'almond extract',
      },
    ],
    categories: 'desserts',
    yield: '~7 dozen',
    equipment:
      '1 1/2- to 2-inch holiday cookie cutters; several small heavy-duty sealable plastic bags (not pleated; if icing cookies)',
    source: "Gourmet, December 2004 (Adapted from Betty Crocker's Cooky Book)",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi35AIM',
    directions:
      'Whisk together flour, baking soda, cream of tartar, and salt in a bowl.\n\nBeat together butter and confectioners sugar in a large bowl with an electric mixer at medium-high speed until pale and fluffy, about 2 minutes in a stand mixer (preferably fitted with paddle attachment) or 4 minutes with a handheld. Beat in egg and extracts. Reduce speed to low, then add flour mixture and mix until combined.\n\nForm dough into 2 balls and flatten each into a 5-inch disk. Chill disks, wrapped in plastic wrap, until firm, at least 1 hour.\n\nPut oven racks in upper and lower thirds of oven and preheat oven to 375°F.\n\nRoll out 1 piece of dough (keep remaining dough chilled) into a 12-inch round (1/8 inch thick) on a well-floured surface with a well-floured rolling pin. (If dough becomes too soft to roll out, chill or freeze on a baking sheet until firm.) Cut out as many cookies as possible from dough with cutters and transfer with a metal spatula to 2 ungreased large baking sheets, arranging cookies about 1 inch apart. Sprinkle cookies with sprinkles or nonpareils if using.\n\nBake cookies, switching position of sheets halfway through baking, until edges are pale golden, 8 to 10 minutes total. Transfer with a metal spatula to racks to cool completely.\n\nGather scraps and chill until firm enough to reroll, 10 to 15 minutes. Make more cookies with remaining dough and scraps (reroll only twice) and bake on cooled sheets.\n\nIf using icing and coloring it, transfer 1/4 cup icing to a small bowl for each color and tint with food coloring. Spoon each color icing (or all of white icing) into a sealable bag, pressing out excess air, and snip an 1/8-inch opening in 1 bottom corner of each bag. Twisting bag firmly just above icing, decoratively pipe icing onto cookies. Let icing dry completely (about 1 hour, depending on humidity) before storing cookies.',
    preptime: 'Active time: 1 hour,  start to finish: 2 1/2 hours',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiuqwgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Meatballs',
    lastModified: '2012-02-03T02:39:53',
    notes: '',
    ingredients: [
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'lb', desc: 'Ground beef' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'Parm-Reggiano',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'Breadcrumbs',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'Garlic powder' },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'Parsley' },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'Oregano' },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'Salt to taste' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'Egg' },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'Jars of sauce' },
    ],
    categories: 'Meat, pasta, main',
    yield: '24 meatballs',
    equipment: '',
    source: 'Jenny Genos',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiuqwgM',
    directions:
      'Oil baking sheet and preheat oven to 450*.\n\nMix all ingredients and make golf ball sized balls.  Place in pan and bake for 20 minutes.\n\nHeat sauce.  Drain grease off meatballs and pour sauce over.  Bake 15 more minutes.  \n\nServe over spaghetti!  Great with cheesy bread.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi--QgM': {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Nutty Granola',
    lastModified: '2012-03-16T17:51:23',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'cups',
        desc: 'rolled oats (not instant)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'pecans, halved',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'other nuts' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'flour' },
      { isGroupHeader: false, qty: 0.333, qty2: 0, uom: 'cup', desc: 'sugar' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp.', desc: 'salt' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp.', desc: 'cinnamon' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'butter, softened',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'honey' },
    ],
    categories: '',
    yield: '4 bags of ~2 cups each',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi--QgM',
    directions:
      '\nPreheat oven to 320 degrees.\n\nMix first 6 ingredients together.  Add butter and honey.\n\nSpread on a baking sheet (it’s helpful if it has raised edges) and bake for 35 minutes, stirring all the way down to the bottom of the pan a few times during baking.  \n\nBake longer for more crunch, but don’t let it burn!  \n\nAllow granola to cool completely after baking (this is when it clumps together).  \n\nServe with milk, yogurt, fruit, ice cream, or just plain snacking!',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOmAQM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Oatmeal Cinnamon Chip Cookies',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'butter or margarine',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'brown sugar',
      },
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'granulated sugar',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'eggs' },
      { isGroupHeader: false, qty: 1, qty2: 0.5, uom: 'tsp.', desc: 'vanilla' },
      { isGroupHeader: false, qty: 1, qty2: 0.5, uom: 'cups', desc: 'flour' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp.',
        desc: 'baking soda',
      },
      { isGroupHeader: false, qty: 2, qty2: 0.5, uom: 'cups', desc: 'oats' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0.667,
        uom: 'cups',
        desc: 'cinnamon chips',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'pecans' },
    ],
    categories: 'cookies',
    yield: 'About 2 - 3 dozen cookies',
    equipment: '',
    source: "Hershey's",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiOmAQM',
    directions:
      '1.  Heat oven to 350°F.\n\n2.  Beat butter and sugars until creamy.  Add eggs and vanilla; beat well.  Combine flour and soda.  Add to butter mixture, beating well.  Stir in oats, cinnamon chips and pecans.\n\n3.  Drop by heaping teaspoons onto ungreased cookie sheet.  Bake 10 - 12 minutes or until lightly browned.  Cool 1 minute; remove from sheet to wire rack.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiAygMM: {
    status: 'active',
    subtitle: "Angie's edited version",
    canEmailOwner: true,
    isFavorite: true,
    title: 'Oatmeal, Chocolate Chip, and Pecan Cookies',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 8,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'unsalted butter, at room temperature',
      },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'light brown sugar, firmly packed',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'teaspoon', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'flour' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'wheat flour',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground cinnamon',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg',
      },
      {
        isGroupHeader: false,
        qty: 0.125,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground clove',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'quick-cooking oats',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chopped pecans',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: 'ounces',
        desc: 'semisweet chocolate chips',
      },
    ],
    categories: 'cookies, desserts',
    yield: '2 - 3 dozen cookies (depending on size)',
    equipment: '',
    source: 'Sparks in the Kitchen',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiAygMM',
    directions:
      'Preheat oven to 350°F. Line a large cookie sheet with parchment or a Silpat (rubber nonstick baking mat).\n\nUsing an electric mixer, beat the butter in a bowl until light and fluffy. Add both sugars, salt, and vanilla, and beat until well mixed, about 3 minutes. Stir in eggs, one at a time.\n\nSift together the flour, baking soda, cinnamon, nutmeg, and clove in a separate bowl.\n\nAdd half of the flour mixture to the butter with the mixer on low speed. Once the flour has been incorporated, add the second half. Stir in the oats, pecans, orange zest, and chocolate chips.\n\nDrop the dough, by the tablespoon, onto the cookie sheet and bake for 10-12 minutes or until golden. Remove from the oven and cool the cookies on a rack.\n\nStore at room temperature in a cookie jar or other airtight container.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhim_gkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Orzo with Spinach Meatballs',
    lastModified: '2013-08-12T02:36:00',
    notes: '',
    ingredients: [
      {
        qty: 0.667,
        qty2: 0,
        uom: 'cup',
        desc: 'finely grated fresh Parmesan',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'pound',
        desc: 'ground turkey or lean ground beef',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: '',
        desc: 'small onion, finely chopped',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'fresh bread crumbs',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'cooked, squeezed dry, and chopped spinach',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large egg, lightly beaten',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Coarse salt', isGroupHeader: false },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'extra-virgin olive oil',
        isGroupHeader: false,
      },
      {
        qty: 8,
        qty2: 0,
        uom: 'ounces',
        desc: 'star-shape pasta (or any small pasta, such as orzo)',
        isGroupHeader: false,
      },
      {
        qty: 4,
        qty2: 0,
        uom: 'cups',
        desc: 'chicken stock or broth',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: 'Serves 6',
    equipment: '',
    source: 'Epicurious',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhim_gkM',
    directions:
      '1. Bring a large pot of salted water to a boil.\n\n2. Set aside 4 tablespoons of the cheese.\n\n3. Combine the ground meat, onion, bread crumbs, spinach, egg, 3/4 teaspoon of the salt, and the remaining cheese in a medium bowl. Shape into 30 meatballs, each 1 inch in diameter.\n\n4. Heat the oil in a large, heavy skillet over medium-high heat until hot but not smoking. Reduce heat to medium-low; add the meatballs and cook, turning, until lightly browned, about 10 minutes. Using a slotted spoon, transfer the meatballs to a large saucepan.\n\n5. Meanwhile, add the pasta (I added extra spinach) to the boiling water and cook according to the package directions; drain.  I added butter and parm.\n\n6. Add the chicken stock (I used pasta water) to the browned meatballs and bring to a boil. Reduce heat and simmer until the meatballs are just cooked through, about 5 minutes. Stir in the pasta and salt to taste.\n\n7. Ladle the soup into bowls and sprinkle with the reserved cheese.\n',
    preptime: '30 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhju5gkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pancakes -- no egg strawberry',
    lastModified: '2013-01-31T03:23:59',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'T',
        desc: 'Butter, melted',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'Banana, mashed',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'C', desc: 'Flour ' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'C',
        desc: 'Wheat flour',
      },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: 'T', desc: 'Sugar' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'Soda' },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'tsp', desc: 'Powder ' },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'tsp', desc: 'Salt' },
      { isGroupHeader: false, qty: 1.5, qty2: 0, uom: 'C', desc: 'Buttermilk' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'Vanilla' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'C',
        desc: 'Strawberries, cut up',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'T', desc: 'Honey' },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhju5gkM',
    directions: '',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhingQkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pasta Salad',
    lastModified: '2012-03-28T01:43:51',
    notes: '',
    ingredients: [
      {
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'white-wine vinegar',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'water',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: 'teaspoons', desc: 'salt', isGroupHeader: false },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'sugar',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'minced fresh tarragon leaves or 1/2 teaspoon dried, crumbled',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc:
          'large garlic clove, minced and mashed to a paste with 1/4 teaspoon salt',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'olive oil',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'pound',
        desc: 'medium pasta shells',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'pound',
        desc:
          'shelled fresh or frozen peas (about 1 1/2 cups), boiled until tender and drained',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'pints',
        desc:
          'red or yellow pear tomatoes or cherry tomatoes or a combination, halved',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'shredded fresh basil leaves',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Gourmet, June 1993',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhingQkM',
    directions:
      'In a large bowl whisk together the vinegar, the water, the salt, the sugar, the tarragon, the garlic paste, and pepper to taste, add the oil in a stream, whisking, and whisk the dressing until it is emulsified. In a kettle of salted boiling water cook the pasta until it is tender, in a colander rinse it well, and drain it. In the bowl toss the pasta with the dressing, add the peas, the tomatoes, and the basil, and toss the salad well.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi11QcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pate brise',
    lastModified: '2011-07-29T22:47:04',
    notes:
      "To ensure a crispy crust, the pastry shell is partially baked before it's filled.",
    ingredients: [
      {
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Pinch of sugar',
        isGroupHeader: false,
      },
      {
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(3/4 stick) chilled butter, cut into 1/2-inch cubes',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc:
          'plus 3/4 teaspoon chilled solid vegetable shortening, cut into 1/2-inch pieces',
        isGroupHeader: false,
      },
      {
        qty: 4,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(or more) cold water',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: 'Makes one 8-inch-diameter crust',
    equipment: '8-inch-diameter springform pan, and dried beans or pie weights',
    source: 'Bon Appétit, August 2009, Julia Child',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi11QcM',
    directions:
      'Whisk flour, salt, and sugar in medium bowl. Add butter and shortening; rub in with fingertips until mixture resembles coarse meal. Add 4 tablespoons cold water. Work mixture with fingertips until dough comes together in moist clumps, adding more water by teaspoonfuls if dry. Gather dough into ball; flatten into disk. Wrap in plastic; chill until firm, at least 1 hour.\n\nPosition rack in center of oven; preheat to 400°F. Roll out dough on floured work surface to 10-inch round. Carefully transfer dough to 8-inch springform pan. Press dough onto bottom and about 1 1/2 inches up sides of pan, pressing to adhere to sides. Fold down and roll 1/2 inch of dough sides inward, forming double-thick edge at top of crust sides. Using dull edge of small knife, make small indentations at 1/2-inch intervals on double-thick edge. Chill 20 minutes.\n\nLine crust with foil; fill with dried beans or pie weights. Bake crust until sides of crust are set, about 18 minutes. Remove foil and beans. Pierce bottom of crust all over with fork. Continue to bake until bottom is set and pale golden, about 14 minutes longer. Remove from oven and cool in pan on rack.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi54QMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pecan Molasses Bundt Cake with Bourbon Glaze',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Cake' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Nonstick vegetable oil spray',
      },
      {
        isGroupHeader: false,
        qty: 3.25,
        qty2: 0,
        uom: 'cups',
        desc: 'cake flour',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: '(2 sticks) unsalted butter, room temperature',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'vanilla extract',
      },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'whole milk' },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'pecans, toasted, finely chopped',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'dark corn syrup',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'mild-flavored (light) molasses',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Glaze' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'water',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'cups', desc: 'sugar' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'buttermilk' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: '(2 sticks) unsalted butter',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'dark corn syrup',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'bourbon' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'vanilla extract',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Vanilla ice cream',
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit, March 2003',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi54QMM',
    directions:
      'For cake:\n\nPreheat oven to 350°F. Spray 10-inch-diameter Bundt pan with nonstick spray; dust with flour. Sift 3 1/4 cups flour, baking powder, and salt into medium bowl. Using electric mixer, beat sugar, butter, and 2 teaspoons vanilla in large bowl until fluffy. Beat in eggs 1 at a time. Beat in flour mixture in 3 additions, alternating with milk in 2 additions. Transfer half of batter to prepared pan.\n\nStir pecans, corn syrup, molasses, and 2 teaspoons vanilla in another medium bowl to blend. Stir in baking soda. Stir pecan mixture into remaining cake batter in bowl. Spoon pecan batter over batter in pan (do not swirl). Bake until tester inserted near center of cake comes out with dry crumbs attached, about 50 minutes. Transfer cake in pan to rack.\n\nMeanwhile, prepare glaze:\n\nStir 2 teaspoons water and baking soda in small bowl to dissolve baking soda. Bring sugar, buttermilk, butter, and corn syrup to boil in heavy 6-quart saucepan over high heat, stirring to dissolve sugar and melt butter. Reduce heat to medium-high. Stir in baking soda mixture (glaze will bubble). Boil until sauce is golden and slightly thickened, stirring often, about 8 minutes. Remove from heat. Stir in bourbon and vanilla.\n\nInvert warm cake onto platter. Immediately brush 1 1/2 cups hot glaze over cake, allowing glaze to soak into cake. Cool cake completely. (Can be made 1 day ahead. Cover and let stand at room temperature. Cover and refrigerate remaining glaze.)\n\nRewarm remaining glaze, stirring. Serve cake with ice cream and warm glaze.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhig5gQM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: true,
    title: 'Pork and Hominy Stew',
    lastModified: '2010-09-28T14:53:17',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc:
          'plus 2 teaspoons chili powder  [can reduce, according to preference]',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'teaspoon', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground black pepper',
      },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'pounds',
        desc:
          'boneless pork shoulder butt, cut into 2 1/2-inch pieces or boneless country pork spareribs, cut into 2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'bacon slices, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large onion, thinly sliced',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'diced smoked ham',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'medium carrot, peeled, chopped',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: '',
        desc: 'large garlic cloves, chopped',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc:
          "poblano chilies,* seeded, cut into 2x1/4-inch strips   [I usually use just one, as it's a bit spicy]",
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'drained canned hominy or sweet corn (from two 15-ounce cans)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'canned diced tomatoes in juice',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc:
          'beer [the darker the better in our house, but you can substitute dry white wine or water]',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'canned low-salt chicken broth',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'dried marjoram',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'chopped fresh cilantro',
      },
    ],
    categories: 'mains, soup, stew, pork',
    yield: '6 Servings',
    equipment: '',
    source: 'Epicurious.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhig5gQM',
    directions:
      'Mix 1 tablespoon chili powder, salt, and pepper in bowl. Rub spice mixture all over pork. Sauté bacon in heavy large pot over medium heat until crisp, about 5 minutes. Transfer bacon to paper towels to drain. Working in batches, add pork to drippings in pot and sauté until brown on all sides, about 10 minutes per batch. Using slotted spoon, transfer pork to bowl.\n\nReduce heat to medium. Add onion, ham, carrot, and garlic to pot; cover and cook 5 minutes, stirring occasionally and scraping up browned bits. Add chilies; stir 1 minute. Stir in hominy, tomatoes with juices, beer, broth, marjoram, pork, and remaining 2 teaspoons chili powder and bring to boil. Reduce heat; cover and simmer until pork is very tender, about 1 hour. (Can be prepared 1 day ahead. Cover and chill bacon. Cool stew slightly. Chill uncovered until cold, then cover and keep chilled.)\n\nSimmer stew uncovered until liquid is slightly reduced and thickened, about 10 minutes. Season with salt and pepper. Transfer to bowls. Sprinkle with reserved bacon and cilantro.\n\nWe like to serve the stew over cornbread, and serve topped with sour cream, avocado, cheese, cilantro, and bacon.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi2zwkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pork and Hominy Stew',
    lastModified: '2013-01-18T18:25:39',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'plus 2 teaspoons chili powder',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'teaspoon', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground black pepper',
      },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'pounds',
        desc:
          'boneless pork shoulder butt, cut into 2 1/2-inch pieces or boneless country pork spareribs, cut into 2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'bacon slices, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large onion, thinly sliced',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'diced smoked ham',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'medium carrot, peeled, chopped',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: '',
        desc: 'large garlic cloves, chopped',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'poblano chilies,* seeded, cut into 2x1/4-inch strips',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'drained canned hominy (from two 15-ounce cans)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'canned diced tomatoes in juice',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'beer' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'canned low-salt chicken broth',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'dried marjoram',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'chopped fresh cilantro',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'avocado' },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: 'sour cream' },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit  | February 2002',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi2zwkM',
    directions:
      'Mix 1 tablespoon chili powder, salt, and pepper in bowl. Rub spice mixture all over pork. Sauté bacon in heavy large pot over medium heat until crisp, about 5 minutes. Transfer bacon to paper towels to drain. \n\nWorking in batches, add pork to drippings in pot and sauté until brown on all sides, about 10 minutes per batch. Using slotted spoon, transfer pork to bowl.\n\nReduce heat to medium. Add onion, ham, carrot, and garlic to pot; cover and cook 5 minutes, stirring occasionally and scraping up browned bits. Add chilies; stir 1 minute. \n\nStir in hominy, tomatoes with juices, beer, broth, marjoram, pork, and remaining 2 teaspoons chili powder and bring to boil. Reduce heat; cover and simmer until pork is very tender, about 1 hour. \n\n(Can be prepared 1 day ahead. Cover and chill bacon. Cool stew slightly. Chill uncovered until cold, then cover and keep chilled.)\n\nSimmer stew uncovered until liquid is slightly reduced and thickened, about 10 minutes. Season with salt and pepper. Transfer to bowl. Sprinkle with reserved bacon, avo, sour cream, and cilantro.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj-twkM': {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pork, Beef, and Black Bean Chili',
    lastModified: '2012-12-31T20:53:40',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'olive oil',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'pound',
        desc: 'ground pork',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'pound',
        desc: 'ground beef',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'pound',
        desc: 'chuck steak, fat trimmed, cut into 1-inch cubes',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: '',
        desc: 'garlic cloves, minced',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'cups', desc: 'water' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'ground cumin',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'chili powder',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'dried oregano',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'teaspoons', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'cayenne pepper',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: '',
        desc: '6-ounce cans tomato paste',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'sugar',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'diced fresh tomatoes',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'diced onions',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'diced red bell peppers',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'canned black beans, drained, rinsed (from three 15-ounce cans)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'chopped fresh cilantro',
      },
      {
        isGroupHeader: false,
        qty: 2.75,
        qty2: 0,
        uom: 'cups',
        desc: '(about) beef broth',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Grated cheddar cheese',
      },
    ],
    categories: 'chili, stew, pork, beans',
    yield: '8-10',
    equipment: '',
    source: ' Bon Appétit  | November 2002  Parker Ranch Grill, Kamuela, HI',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj-twkM',
    directions:
      ' Heat olive oil in heavy large pot over medium-high heat. Add ground pork and ground beef and sauté until brown, about 3 minutes. Add cubed beef and garlic and sauté 5 minutes. Add 2 cups water; bring to boil. Add cumin, chili powder, oregano, salt, and cayenne pepper. Reduce heat, cover, and simmer 10 minutes. Add tomato paste and sugar and simmer 5 minutes. Add tomatoes, onions, and peppers and simmer until vegetables are tender, about 30 minutes.\n\nAdd black beans and cilantro to chili. Add beef broth 3/4 cup at a time, until thinned to desired consistency. (Can be prepared 1 day ahead. Refrigerate uncovered until cold, then cover and keep refrigerated.) Bring chili to simmer. Ladle hot chili into bowls; top with grated cheddar cheese and serve.\n\n',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiVvAUM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Potato Soup',
    lastModified: '2010-09-28T14:53:18',
    notes: '',
    ingredients: [
      { isGroupHeader: false, qty: 1, qty2: 2, uom: 'sticks', desc: 'butter' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'medium onions, finely sliced',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large leek, finely sliced',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'large stick celery, finely sliced',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 2,
        uom: 'lbs.',
        desc: 'potatoes, finely sliced (or cubed to save time)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'liter',
        desc: 'Light chicken stock',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Maldon sea salt',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 1,
        uom: 'c.',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'small bunch of chives, chopped',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Bacon and grated cheese for topping',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Watercress (optional)',
      },
    ],
    categories: 'potatoes, soup, mains,  vegetables,',
    yield: '4+ servings',
    equipment: '',
    source: 'Harvey Nichols',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiVvAUM',
    directions:
      'Melt the butter in a saucepan; add the vegetables, cover and sweat on a very low heat for 45 minutes to an hour, or until the vegetables are completely soft.  Add the stock and bring to a boil; simmer gently for a further 15 minutes, no more. Season with sea salt.  (Or, for a quicker version, saute vegetables until translucent, then add stock and boil until potatoes are tender.)\n\nLiquidize in small batches until completely smooth, then pass the mixture through a fine sieve. In the restaurant we would us a chinoise sieve. This is hard work, but the resulting texture is as good as liquid velvet and is what turns this soup from something good into something remarkable.\n\nAlternatively, for a chunkier soup, only puree 3/4 of the soup and mix it with the other 1/4.\n\nStir in the cream and reheat as close to boiling as possible, but stop short or you run the risk of curdling the soup.\n\nServe with a garnish of chopped chives, watercress, bacon and cheese.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiYxQIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Potato and Parmesan Gratin',
    lastModified: '2010-09-28T14:53:15',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'lbs',
        desc: 'medium boiling potatoes',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'fine sea salt',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy cream',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'whole milk' },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'unsalted butter, softened',
      },
      {
        isGroupHeader: false,
        qty: 7,
        qty2: 0,
        uom: 'oz',
        desc: 'finely grated Parmigiano-Reggiano (3 1/2 cups)',
      },
    ],
    categories: 'sides, vegetables, potatoes, casseroles',
    yield: '',
    equipment: '',
    source: 'Epicurious',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiYxQIM',
    directions:
      'Put oven rack in middle position and preheat oven to 350°F.\n\nPeel potatoes and cut into 1/8-inch-thick slices with a knife or an adjustable-blade slicer, spreading slices out on a large kitchen towel. Sprinkle with sea salt (1 1/2 teaspoons).\n\nStir together cream and milk.\n\nDot bottom of a 3-quart shallow baking dish (13 by 9 inches) with half of butter and pour in 1/3 cup cream mixture. Without rinsing or drying potatoes, divide into 5 piles. Layer potatoes in baking dish, 1 pile per layer, spreading 1/3 cup cream mixture and one fourth of cheese between layers. Drizzle remaining cream mixture over potatoes and dot with remaining butter.\n\nBake, uncovered, until potatoes are very tender and top is browned, about 2 hours. Let stand at room temperature 10 minutes before serving.',
    preptime: '20 min. active, 2 hours baking time',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjelQoM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pumpkin Bars',
    lastModified: '2013-10-23T14:14:36',
    notes: '',
    ingredients: [
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: 'cups', desc: 'sugar', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'ground cinnamon',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'teaspoon', desc: 'salt', isGroupHeader: false },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground cloves',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '15-ounce can solid pack pumpkin',
        isGroupHeader: false,
      },
      { qty: 4, qty2: 0, uom: '', desc: 'large eggs', isGroupHeader: false },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'vegetable oil (or un-butter)',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'raisins (or 1/2 cup)',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'May add pecans, or top with them',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Frosting:', isGroupHeader: true },
      {
        qty: 8,
        qty2: 0,
        uom: 'ounces',
        desc: 'cream cheese, room temperature',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'powdered sugar',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'butter, room temperature',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: 'a lot',
    equipment: '',
    source: 'Bon Appétit, January 1998',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjelQoM',
    directions:
      'Preheat the oven to 350°F. Grease 15 1/2 x 10 1/2 x 1-inch baking sheet. Stir first 8 ingredients in large bowl to blend. Add pumpkin, eggs and oil and beat until blended. Mix in raisins. Spread batter in prepared pan. Bake until tester inserted into center comes out clean, about 25 minutes. Cool in pan on rack.\nBeat cream cheese, powdered sugar and butter in medium bowl to blend. Spread frosting over cake in thin layer. (Can be prepared 1 day ahead. Refrigerate until cold, then cover and keep refrigerated.) Cut cake into bars and serve.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiK9wEM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pumpkin Buttermilk Pancakes',
    lastModified: '2010-09-28T14:53:15',
    notes: '',
    ingredients: [
      { isGroupHeader: false, qty: 1.5, qty2: 0, uom: 'cups', desc: 'flour' },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: 'T', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tsp',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'baking soda',
      },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'tsp',
        desc: 'pumpkin pie spice',
      },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'cup', desc: 'pumpkin' },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'buttermilk',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'eggs, may separate and beat whites',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'T',
        desc: 'butter, melted',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'vanilla' },
    ],
    categories: 'breakfast, breads, pumpkin',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiK9wEM',
    directions:
      'Whisk together dry ingredients.\n\nAdd rest of ingredients (may fold in whites last, if separated).  Do not overmix.\n\nHeat griddle or pan and make pancakes!',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjNzQcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pumpkin Pasta',
    lastModified: '2011-07-23T01:24:36',
    notes: '',
    ingredients: [
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Puree in blender and set aside:',
        isGroupHeader: true,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'roasted sugar pumpkin (1-2 cups)',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: '~1 cup Chicken broth',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: '~1/2 cup cream cheese',
        isGroupHeader: false,
      },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Sauté and salt to taste:',
        isGroupHeader: true,
      },
      { qty: 0, qty2: 0, uom: '', desc: '~2 T butter', isGroupHeader: false },
      {
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Generous amount of olive oil',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'red onion, sliced',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'shallots, sliced',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc:
          'carrot, very thinly sliced (or a handful of baby carrots thinly sliced)',
        isGroupHeader: false,
      },
      {
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'or 4 garlic cloves, minced',
        isGroupHeader: false,
      },
    ],
    categories: 'pumpkin, pasta',
    yield: '',
    equipment: '',
    source: "Angie's head",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjNzQcM',
    directions:
      'While sautéing, boil pasta (I used half rigatoni and half macaroni).  Drain and reserve.\n\nOnce sauté is mostly cooked, add a handful of spinach and cook until wilted.\n\nPour blended pumpkin mixture over sauté.  Heat through, and bring just to a simmer.  Pour over pasta.  Top with parmiggano reggiano or Parmesan.  Salt and pepper to taste!',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiG3wkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pumpkin Pie with Ginger Streusel',
    lastModified: '2013-01-18T18:37:27',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'For crust' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.667,
        qty2: 0,
        uom: 'cup',
        desc: 'finely chopped pecans',
      },
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'cake flour',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(packed) golden brown sugar',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 5,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'chilled unsalted butter, cut into 1/2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'chilled vegetable shortening, cut into 1/2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(about) ice water',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Filling' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '16-ounce can solid pack pumpkin',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'whipping cream',
      },
      { isGroupHeader: false, qty: 3, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'packed golden brown sugar',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'ground cinnamon',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'ground allspice',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground cloves',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Topping' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'packed golden brown sugar',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'coarsely chopped walnuts (about 2 1/2 ounces)',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'finely chipped crystallized ginger',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'ground ginger',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc:
          '(1 stick) unsalted butter, cut into small pieces, room temperature',
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit  | November 1994',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiG3wkM',
    directions:
      'For crust:\n\nPlace ginger in processor and process; until minced. Add flour, sugar, cloves and salt; process to combine. Using on/off turns, cut in butter and shortening until mixture resembles coarse meal. Mix yolk and 2 tablespoons water in small bowl. Add yolk mixture to flour and butter mixture; process just until mixture forms moist clumps. If dough is too dry, blend in more water by teaspoonfuls. Gather dough into ball; flatten into disk. Wrap in plastic; chill 1 hour. (Can be made 1 day ahead. Keep chilled. Soften dough slightly at room temperature before rolling.)\nBake until pale golden @350\n\nFor filling:\nWhisk all ingredients in large bowl until combined. Pour into pie crust. Bake until skin begins to form on filling and filling begins to set, about 50 minutes. Remove from oven. Let pie stand 10 minutes to set slightly. Maintain oven temperature.\n\nMeanwhile, prepare topping:\nMix first 5 ingredients in medium bowl. Rub in butter with fingertips until mixture begins to form small clumps. Sprinkle topping over pie. Bake until pie is set and streusel is golden brown, about 25 minutes. Transfer to rack and cool completely.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhimhAgM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Pumpkin Stew',
    lastModified: '2011-12-09T23:23:39',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'onions, chopped',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'cloves garlic, minced',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'T', desc: 'oil' },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'T', desc: 'butter' },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'T',
        desc: 'brown sugar',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'tsp', desc: 'cumin' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'chili powder',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'tsp', desc: 'nutmeg' },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'cups',
        desc: 'chicken stock',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'milk or cream',
      },
      {
        isGroupHeader: false,
        qty: 30,
        qty2: 0,
        uom: 'oz',
        desc: 'pumpkin purée',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'cans corn' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'cans black beans',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'of cilantro, chopped',
      },
    ],
    categories: 'Soup, stew, main, pumpkin',
    yield: '6-8',
    equipment: '',
    source: 'Adapted from epicurious',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhimhAgM',
    directions:
      'Sauté first 5 ingredients in large soup pot at low to med heat. Cook 1/2 hr so that the onions slightly caramelize. \n\nAdd the spices and incorporate. Add the broth and milk or cream  and allow to come to a simmer. When hot whisk in the pumpkin.\n\nAdd corn, black beans, and cilantro.  \n\nTop with sour cream, cheese, cilantro and avocado.',
    preptime: '30 min-1 hour',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjV9AcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Quick Chicken Chili',
    lastModified: '2011-09-09T02:08:32',
    notes: '',
    ingredients: [
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'T', desc: ' olive oil' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'Chicken breasts',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'onion, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'pepper, hatch, poblano or jalapeño',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'cloves of garlic, minced',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'tomatoes, chopped (or a can of diced/crushed)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'can red kidney beans, drained and rinsed',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'can corn, drained',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'chicken broth',
      },
      { isGroupHeader: false, qty: 0.333, qty2: 0, uom: 'cup', desc: 'salsa' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Cumin, chili powder, salt, and pepper to taste',
      },
    ],
    categories: 'chicken, chili',
    yield: '4-6',
    equipment: '',
    source: 'Angie',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjV9AcM',
    directions:
      'Sear chicken in oil.  Reserve.\n\nSauté veg.  Add canned veg.  Chop chicken and add. \n\nAdd spices and broth.  Stir in salsa to taste.\n\nCook 10 minutes or so.\n\nServe with cornbread, rice, sour cream or cheese.',
    preptime: '30 min',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiVtgcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Ribs',
    lastModified: '2011-06-04T20:17:05',
    notes:
      'Debra L. Warren of Lake Forest, Illinois, writes: "I have often indulged in Jay Lovell\'s phenomenal cuisine at his Lovell\'s of Lake Forest restaurant, and the ribs he prepared for an event I attended were out of this world. Could you possibly persuade the chef to share the recipe?"\n\nThe espresso powder gives the sauce great depth of flavor.\n',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'hot Mexican-style chili powder',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'paprika',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'ground cumin',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground black pepper',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'pounds',
        desc: 'baby back pork ribs',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '12-ounce bottle dark beer',
      },
      { isGroupHeader: false, qty: 0, qty2: 0, uom: '', desc: '' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '18-ounce bottle good-quality barbecue sauce',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'cup', desc: 'water' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(packed) golden brown sugar',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'instant espresso powder',
      },
    ],
    categories: '',
    yield: 'Makes 4 to 6 servings',
    equipment: '',
    source: "Lovell's of Lake Forest restaurant (via Bon Appétit, June 2003)",
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiVtgcM',
    directions:
      'Preheat oven to 400°F. Whisk chili powder, paprika, cumin, salt, and pepper in small bowl to blend. Rub spice mixture all over ribs. Place ribs in heavy large roasting pan.\n\nBoil beer in medium saucepan until reduced to 1 cup, about 5 minutes. Pour beer around ribs. Cover pan tightly with foil. Bake ribs until fork-tender, about 1 hour 30 minutes.\n\nCombine barbecue sauce, 1/2 cup water, brown sugar, and espresso powder in heavy medium saucepan. Simmer until flavors blend and sauce thickens slightly, stirring occasionally, about 10 minutes. (Ribs and barbecue sauce can be prepared 1 day ahead. Cool slightly, then cover separately and refrigerate.)\n\nPrepare barbecue (medium heat). Brush ribs with some barbecue sauce. Grill ribs just until heated through, about 3 minutes per side. Transfer ribs to platter. Bring remaining sauce to simmer. Brush ribs with more barbecue sauce. Serve ribs, passing remaining barbecue sauce separately.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjGoAkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: "S'mores Cupcakes",
    lastModified: '2012-09-20T21:46:42',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2.25,
        qty2: 0,
        uom: 'cups',
        desc: 'plus 2 tablespoons sugar',
      },
      {
        isGroupHeader: false,
        qty: 1.75,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'plus 1 tablespoon cocoa powder (not Dutch-processed)',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking soda',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'teaspoon', desc: 'salt' },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'whole milk' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'vegetable oil',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'pure vanilla extract',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'boiling water',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'graham cracker crumbs (from about 20 squares)',
      },
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'unsalted butter, melted',
      },
      {
        isGroupHeader: false,
        qty: 9,
        qty2: 0,
        uom: 'ounces',
        desc: 'bittersweet chocolate, finely chopped',
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjGoAkM',
    directions:
      "S'MORES CUPCAKES WITH                        \nTOASTED MARSHMALLOW ICING   \nMakes 2 Dozen\nIngredients\nMakes 2 dozen\n\nMarshmallow Frosting\nDirections\n\nPreheat oven to 350 degrees. Line 2 standard muffin tins with cupcake liners; set aside.\n\nSift 2 cups plus 2 tablespoons sugar, flour, cocoa powder, baking powder, baking soda, and salt together into the bowl of an electric mixer. Using the paddle attachment, mix ingredients together on low speed.\nIn a large bowl, mix together eggs, milk, oil, and vanilla. Add to flour mixture and beat on medium speed for 30 seconds. Scrape down sides of bowl and continue mixing on medium speed for 2 minutes. Add boiling water and stir to combine; set cake batter aside.\n\nPlace graham cracker crumbs, remaining 1/4 cup sugar, and melted butter in a large bowl; stir until well combined.\n\nPlace 1 tablespoon graham cracker mixture into the bottom of each prepared muffin cup. Use the bottom of a small glass to pack crumbs into the bottom of each cupcake liner. Reserve remaining graham cracker mixture for topping.\n\nPlace 2 teaspoons chocolate in each muffin cup. Transfer muffin tins to oven and bake until the edges of the graham cracker mixture is golden, about 5 minutes. Remove from oven and fill each muffin cup three-quarters full with cake batter. Sprinkle each with remaining chocolate and graham cracker mixture. Return to oven and bake, rotating pans halfway through baking, until tops are firm and a cake tester inserted in the center comes out clean, 18 to 20 minutes. Transfer muffin tins to a wire rack and let cupcakes cool in pan for 10 minutes. Remove cupcakes from pan and let cool completely.\n\nTransfer frosting to a large pastry bag fitted with a large plain round or French tip (such as Ateco #867 or Ateco #809). Pipe frosting in a spiral motion on each cupcake. Transfer cupcakes to a baking sheet. Using a kitchen torch, lightly brown the frosting, taking care not to burn the cupcake liners. Serve immediately or store in an airtight container, up to 2 days.\n\nMARSHMALLOW ICING\n8 large egg whites\n2 cups sugar\n1/2 teaspoon cream of tartar\n2 teaspoons pure vanilla extract\n\nDirections\n\nPlace egg whites, sugar, and cream of tartar in the heatproof bowl of an electric mixer. Set over a saucepan with simmering water. Whisk constantly until sugar is dissolved and whites are warm to the touch, 3 to 4 minutes.\nTransfer bowl to electric mixer fitted with the whisk attachment, and beat, starting on low speed, gradually increasing to high, until stiff, glossy peaks form, 5 to 7 minutes. Add vanilla, and mix until combined. Use immediately.",
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhie1wkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Spiced Layer Cake with Orange Cream Cheese Frosting',
    lastModified: '2013-01-18T18:28:47',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground ginger',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground cardamom',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground cinnamon',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: '(2 sticks) unsalted butter, room temperature',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'grated orange peel',
      },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sour cream' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'whole milk',
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Bon Appétit  | October 1999',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhie1wkM',
    directions:
      ' Preheat oven to 350°F. Butter three 9-inch-diameter cake pans. Line bottom of pans with waxed paper. Butter and flour pans; tap out excess flour. \n\nSift first 8 ingredients into medium bowl. Using electric mixer, beat sugar, butter and orange peel in large bowl until fluffy. Beat in eggs 1 at a time. Stir flour mixture into butter mixture alternately with sour cream and milk. Divide batter among prepared pans.\n\nBake cakes until tester inserted into center comes out clean, about 25 minutes. Transfer pans to racks and cool 10 minutes. Turn cakes out onto racks and cool completely. (Can be made 1 day ahead. Store airtight at room temperature.)\n\nPlace 1 cake layer on platter. Spread 2/3 cup frosting over. Top with second cake layer. Spread 2/3 cup frosting over. Top with third cake layer. Cover top and sides of cake with remaining frosting. (Can be made 1 day ahead. Cover with cake dome and store at room temperature.)\n',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhid3QcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Strawberry Rhubarb Coffeecake',
    lastModified: '2011-09-03T13:57:42',
    notes: '',
    ingredients: [
      { qty: 0, qty2: 0, uom: '', desc: 'Filling:', isGroupHeader: true },
      { qty: 0.667, qty2: 0, uom: 'cup', desc: 'sugar', isGroupHeader: false },
      {
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'cornstarch',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chopped rhubarb',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '(10 ounce) package frozen sliced strawberries, thawed',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'lemon juice',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Cake:', isGroupHeader: true },
      {
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
        isGroupHeader: false,
      },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'sugar', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'cold butter or margarine',
        isGroupHeader: false,
      },
      { qty: 2, qty2: 0, uom: '', desc: 'eggs', isGroupHeader: false },
      { qty: 1, qty2: 0, uom: 'cup', desc: 'buttermilk', isGroupHeader: false },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Topping:', isGroupHeader: true },
      { qty: 0.75, qty2: 0, uom: 'cup', desc: 'sugar', isGroupHeader: false },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'all-purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'cold butter or margarine',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: '',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhid3QcM',
    directions:
      'Combine sugar and cornstarch in a large saucepan; stir in rhubarb and strawberries and bring to a simmer over medium heat. Cook until thickened, about two minutes. Remove from heat, stir in lemon juice and let cool.\nPreheat oven to 350 degrees F (175 degrees C). Lightly grease a 9x13 inch pan.\nCombine 3 cups flour, 1 cup sugar, baking powder, and baking soda in a large bowl. Cut in butter until mixture resembles coarse crumbs. Beat together eggs, buttermilk, and vanilla in a separate bowl. Stir egg mixture into flour until just moistened. Spoon two-thirds of the batter into prepared pan; evenly spread on the cooled filling, then cover with remaining batter.\nIn a small bowl, combine 3/4 cup sugar and 1/2 cup flour, and cut in butter until the mixture resembles coarse crumbs; sprinkle over the batter. Bake in preheated oven for 45 to 50 minutes or until golden brown. Cool on a wire rack.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjlywUM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Sugar Cookie Icing',
    lastModified: '2010-09-28T14:53:18',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 3.75,
        qty2: 0,
        uom: 'cups',
        desc: 'powdered sugar',
      },
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'large egg whites',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'water',
      },
    ],
    categories: 'desserts',
    yield: '',
    equipment: '',
    source: 'Magpies - Bellville, Texas',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjlywUM',
    directions: 'Mix for 6 minutes.\n\nAdd colors!',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiG4ggM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Sweet Potato Fries',
    lastModified: '2012-02-27T03:54:06',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: -1,
        qty2: 3,
        uom: '',
        desc: 'Sweet Potatoes, 1/2" slices',
      },
      {
        isGroupHeader: false,
        qty: -1,
        qty2: 3,
        uom: '',
        desc: 'Garlic cloves, minced',
      },
      { isGroupHeader: false, qty: -1, qty2: 3, uom: 'tsp', desc: 'olive oil' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Salt and Pepper',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Thyme and Parsley',
      },
    ],
    categories: '',
    yield: '',
    equipment: '',
    source: 'Epicurious/Angie',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiG4ggM',
    directions:
      'Toss garlic and potatoes in oil, salt and pepper.\n\nBake at 500 degrees for 30 minutes, turning every 5 minutes.\n\nSprinkle with herbs.',
    preptime: '45 minutes',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjt7AcM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Tomato and Cheddar Pie',
    lastModified: '2011-09-08T21:28:26',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'kosher salt',
      },
      {
        isGroupHeader: false,
        qty: 6,
        qty2: 0,
        uom: 'tablespoons',
        desc: '(3/4 stick) chilled unsalted butter, cut into 1/2" cubes',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'buttermilk' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'pounds',
        desc: 'large ripe tomatoes, cored and cut into 1/4" slices',
      },
      {
        isGroupHeader: false,
        qty: 2.5,
        qty2: 0,
        uom: 'cups',
        desc: 'coarsely grated extra-sharp cheddar (89 ounces)',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'finely grated Parmesan (1/2 ounce)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'scallion, trimmed, chopped',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'mayonnaise',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'chopped fresh dill',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'apple cider vinegar',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'sugar',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'kosher salt',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'freshly ground black pepper',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'cornmeal',
      },
    ],
    categories: '',
    yield: '6-8',
    equipment: '',
    source: 'Bon Appétit, August 2011, Ian Knauer',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjt7AcM',
    directions:
      'Yield: Makes 8 servings\nWith its biscuity buttermilk crust, this rustic pie is our new summertime staple. Let the pie cool for at least one hour before serving.\n\nIngredients\nCrust:\n2 cups all-purpose flour\n1 1/2 teaspoons baking powder\n1/2 teaspoon baking soda\n1/2 teaspoon kosher salt\n6 tablespoons (3/4 stick) chilled unsalted butter, cut into 1/2" cubes\n1 cup buttermilk\n\nFilling:\n2 pounds large ripe tomatoes, cored and cut into 1/4" slices\n2 1/2 cups coarsely grated extra-sharp cheddar (89 ounces)\n1/4 cup finely grated Parmesan (1/2 ounce)\n1 scallion, trimmed, chopped\n1/2 cup mayonnaise\n2 tablespoons chopped fresh dill\n1 tablespoon apple cider vinegar\n2 teaspoons sugar\n3/4 teaspoon kosher salt\n1/2 teaspoons freshly ground black pepper\n1 1/2 tablespoons cornmeal\n\nSpecial Equipment: Use a 9"-diameter glass or ceramic pie dish\n\n\nPreparation\nFor crust:\nWhisk first 4 ingredients in a medium bowl. Using your fingertips, rub in butter until coarse meal forms and some small lumps remain. Stir in buttermilk and knead gently with your hands until dough forms. Wrap dough in plastic and chill for 1 hour.\n\nFor filling:\nLay tomatoes in a single layer on a baking sheet lined with 2 layers of paper towels. Place another 2 layers of paper towels on top of tomatoes. Let stand for 30 minutes to drain.\n\nPreheat oven to 425°F. Roll out dough between 2 sheets of plastic wrap to an 11" round. Remove top layer of plastic wrap. Invert dough onto pie dish. Carefully peel off plastic wrap.\n\nToss both cheeses in a medium bowl until evenly incorporated. Reserve 1/4 cup of cheese mixture. Whisk scallion, mayonnaise, dill, vinegar, sugar, salt, and pepper in a small bowl.\n\nSprinkle cornmeal evenly over bottom of crust, then top with 1/2 cup cheese mixture. Arrange 1/3 of tomatoes over cheese, overlapping as needed. Spread half of mayonnaise mixture (about 1/3 cup) over. Repeat layering with 1 cup of cheese mixture, 1/2 of remaining tomato slices, and remaining mayonnaise mixture. Sprinkle remaining 1 cup cheese mixture over, then remaining tomato slices. Sprinkle with reserved 1/4 cup cheese mixture. Fold overhanging crust up and over edges of tomato slices.\n\nBake pie until crust is golden and cheese is golden brown, 35-40 minutes (check crust halfway and tent with foil if it\'s getting too dark). Let pie cool at least 1 hour and up to 3 hours before slicing and serving.',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhimgQkM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Tres Leches Cake',
    lastModified: '2012-03-28T01:31:40',
    notes: '',
    ingredients: [
      { qty: 0, qty2: 0, uom: '', desc: 'Cake:', isGroupHeader: true },
      {
        qty: 6,
        qty2: 0,
        uom: '',
        desc: 'large eggs, separated',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'granulated sugar',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'all-purpose flour',
        isGroupHeader: false,
      },
      {
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'baking powder',
        isGroupHeader: false,
      },
      {
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'whole milk',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Cream topping:', isGroupHeader: true },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '14-ounce can evaporated milk',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: '14-ounce can sweetened condensed milk',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy cream',
        isGroupHeader: false,
      },
      { qty: 0, qty2: 0, uom: '', desc: 'Icing:', isGroupHeader: true },
      {
        qty: 3,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'water',
        isGroupHeader: false,
      },
      {
        qty: 0.75,
        qty2: 0,
        uom: 'cup',
        desc: 'granulated sugar',
        isGroupHeader: false,
      },
      {
        qty: 3,
        qty2: 0,
        uom: '',
        desc: 'large egg whites',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'ripe mango, peeled, seed removed, and thinly sliced',
        isGroupHeader: false,
      },
      {
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'ripe papaya, peeled, seeds removed, and thinly sliced',
        isGroupHeader: false,
      },
    ],
    categories: '',
    yield: 'About 10 servings',
    equipment: '',
    source: 'Emeril',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhimgQkM',
    directions:
      'To make the cake: Preheat the oven to 350 degrees F. Lightly grease and flour a 9 by 13-inch baking dish and set aside.\nIn the bowl of a mixer, beat the egg whites on low speed until soft peaks form. Add the sugar gradually with the mixer running and peak to stiff peaks. Add the egg yolks 1 at a time, beating well after the addition of each.\nSift together the flour and baking powder and add to the egg mixture, alternating with the milk. (Do this quickly so the batter does not lose volume.) Add the vanilla. Bake until golden, 25 minutes.\nTo make the cream topping: In a blender, combine the evaporated milk, condensed milk, and heavy cream and blend on high speed.\nRemove the cake from the oven and while still warm, pour the cream mixture over it. Let sit and cool to room temperature. Cover and refrigerate until well chilled, at least 4 hours or overnight.\nTo make the icing: Once the cake is completely chilled, in a saucepan combine the water and sugar. Bring to a boil. Reduce the heat and stir to dissolve the sugar. Cook until the mixture reaches the soft ball stage, 235 to 240 degrees F. Remove from the heat. In a medium bowl, beat the egg whites to soft peaks. While beating, add the hot syrup in a stream. Beat until all the syrup has been added, the mixture cools, and a glossy icing forms.\nTo assemble: Remove the cake from the refrigerator and spread the icing evenly across the top. Arrange the mango and papaya slices over the top and serve.',
    preptime: '1 hr (2 hrs total)',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhie2ggM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Tuna Sandwiches',
    lastModified: '2012-02-26T20:50:27',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: -1,
        qty2: 0,
        uom: 'T',
        desc: 'Miracle Whip',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: '', desc: 'Cans of Tuna' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'olive oil' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'lemon juice',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Salt and Pepper to taste',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: '',
        desc: 'Small Apple, chopped',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'Boiled Eggs, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'Stick of Celery and top, chopped',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'Handful chopped Pecans and/or Sliced Almonds',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'serve with avocado slices, cucumber, and greens',
      },
    ],
    categories: 'Lunch, sandwich',
    yield: '',
    equipment: '',
    source: 'Angie and Magpies',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhie2ggM',
    directions:
      'Mix up first 5 ingredients.  Add the rest.  Spread on whole wheat bread. Crop with avo, cuc, and lettuces. :)',
    preptime: '',
    owner_id: '102168863463009654270',
    owner_email: 'angieboone01@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiolgIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: true,
    title: 'Cinnamon Ice Cream',
    lastModified: '2010-09-18T04:07:18',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 3,
        qty2: 0,
        uom: 'cups',
        desc: 'whole milk',
      },
      { isGroupHeader: false, qty: 1.25, qty2: 0, uom: 'cups', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 5,
        qty2: 0,
        uom: '',
        desc: 'cinnamon sticks',
      },
      {
        isGroupHeader: false,
        qty: 0.125,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'salt',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: '',
        desc: 'large egg yolks',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
      },
    ],
    categories: 'desserts',
    yield: 'Makes about 7 cups',
    equipment: '',
    source: 'Bon Appétit, November 2002',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiolgIM',
    directions:
      "Combine milk, 1 cup sugar, cream, cinnamon sticks, and salt in heavy large saucepan. Bring almost to simmer over medium-high heat. Whisk egg yolks and 1/4 cup sugar in large bowl. Gradually whisk in hot milk mixture. Return custard to saucepan and stir constantly over low heat until custard thickens, about 4 minutes (do not boil). Transfer custard to large bowl set over another large bowl of ice water; let cool 30 minutes, stirring often. Cover and chill custard overnight.\n\nStrain custard. Process in ice cream maker according to manufacturer's instructions. Transfer ice cream to container; cover and freeze.",
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhif5gQM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Cream of Vegetable Soup',
    lastModified: '2010-09-18T04:08:02',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'water, vegetable or chicken stock',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'unsalted butter',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'medium onion, coarsely chopped',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'cloves garlic, minced',
      },
      {
        isGroupHeader: false,
        qty: 0.125,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg (optional)',
      },
      { isGroupHeader: false, qty: 4.5, qty2: 0, uom: 'cups', desc: 'stock' },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'cups',
        desc: 'white wine',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'pounds',
        desc: 'vegetable of choice, coarsely chopped',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: '(cauliflower, broccoli, carrot, spinach, etc.)',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0.5,
        uom: 'cup',
        desc: 'heavy cream or milk',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 1,
        uom: 'teaspoon',
        desc: 'ground white or black pepper',
      },
    ],
    categories: '',
    yield: 'Serves 4-6',
    equipment: '',
    source: 'The Joy of Cooking',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhif5gQM',
    directions:
      'Heat stock and butter in a large heavy saucepan or soup pot over medium-low until butter is melted.Add onion, garlic, and nutmeg, and cook, stirring occasionally until tender but not browned. Stir in stock, wine, and vegetables. Bring to a boil, reduce heat, and simmer until veg is tender, 15 – 20 minutes. Puree until smooth.\n\nReturn to the pot and stir in cream, salt, and pepper. Simmer briefly and ladle into warmed bowls. Garnish with herbs and serve with croutons or crackers.',
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjBjgIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Espresso Chocolate Cheesecake Harris',
    lastModified: '2010-09-24T15:49:39',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'For filling' },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
      { isGroupHeader: false, qty: 0.75, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'pounds',
        desc: 'cream cheese, softened',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'cornstarch',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'plus 1 teaspoon vanilla extract',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sour cream' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'instant espresso powder',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: 'ounces',
        desc:
          'fine-quality bittersweet chocolate (not unsweetened), melted and cooled',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'chilled heavy cream, whipped into soft peaks',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Garnish: bittersweet and milk chocolate curls',
      },
    ],
    categories: 'desserts, cheesecakes',
    yield: '',
    equipment: '',
    source: 'Gourmet, January 1997',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjBjgIM',
    directions:
      'Make crust:\nPreheat oven to 325°F. and butter a 9-inch springform pan.\n\nIn a bowl stir together wafer crumbs and butter. Press mixture onto bottom and 1 inch up side of pan. Bake crust in middle of oven 10 minutes and cool in pan on a rack.\n\nWhen crust is cool, wrap pan in foil by putting in middle of am 18-inch square of heavy-duty foil and crimping foil around bottom and up side of pan to secure.\n\nMake filling:\nIn a large bowl with an electric mixer beat eggs with sugar until light and fluffy. Add cream cheese and beat until smooth. Sift cornstarch into mixture and add 1 teaspoon vanilla and sour cream. Beat filling until smooth and transfer 2 cups to a small bowl.\n\nIn a small cup stir together espresso powder and remaining tablespoon vanilla, stirring until espresso powder is dissolved. Stir espresso mixture into filling in small bowl.\n\nAdd chocolate to filling in large bowl, stirring until combined well, and fold in whipped cream. Pour chocolate filling into crust. Spoon espresso filling evenly over chocolate filling and spread to form an even layer, being careful not to mix layers.\n\nPut springform pan in a larger shallow baking pan and pour enough hot water to reach halfway up side of springform pan. Bake cheesecake in middle of oven 1 1/2 hours. Turn off oven and let cheesecake stand 15 minutes with oven door closed. Transfer cheesecake in pan to a rack and cool completely. Chill cheesecake, covered, at least 1 day and up to 2. Run a thin knife around edge of pan and remove side.\n\nGarnish cheesecake with chocolate curls. To serve, cut cheesecake with a sharp knife dipped in hot water.',
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiwvQIM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Hazelnut Praline Cheesecake',
    lastModified: '2010-09-18T04:08:42',
    notes: '',
    ingredients: [
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Crust' },
      {
        isGroupHeader: false,
        qty: 1.25,
        qty2: 0,
        uom: 'cups',
        desc: 'cake flour',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc:
          'plus 2 tablespoons (1 1/4 sticks) chilled unsalted butter, cut into 1/2-inch pieces',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: '(packed) golden brown sugar',
      },
      {
        isGroupHeader: true,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Hazelnut Praline Filling',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: '8-ounce packages cream cheese, room temperature',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'vanilla extract',
      },
      { isGroupHeader: false, qty: 4, qty2: 0, uom: '', desc: 'large eggs' },
    ],
    categories: 'desserts, cheescakes',
    yield: '',
    equipment: '',
    source: 'Bon Appétit, February 1998',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhiwvQIM',
    directions:
      'For crust:\n\nPosition rack in center of oven; preheat to 350°F. Butter 9-inch-diameter springform pan with 2 3/4-inch-high sides. Mix flour, butter and brown sugar in processor until moist clumps form. Press mixture onto bottom of prepared pan. Bake until crust is golden, about 25 minutes. Cool. Wrap outside of pan with double layer of heavy-duty foil.\n\nFor filling:\n\nReserve 1/2 cup praline for garnish. Grind remaining praline to powder in processor. Beat cream cheese and su gar in large bowl until smooth. Beat in flour, then cream and vanilla. Beat in eggs 1 at a time. Fold in praline powder.\n\nTransfer filling to crust. Place springform pan in roasting pan. Pour enough hot water into roasting pan to come halfway up sides of springform pan. Bake until center is just set, about 1 hour 20 minutes.\n\nTransfer cake to rack. Run small sharp knife around sides of cake to loosen. Cool completely. Remove foil from pan sides. Chill cake overnight. (Can be prepared 3 days ahead. Cover and keep refrigerated.)\n\nRelease pan sides. Transfer cake to platter. Mound reserved coarsely crushed praline atop center of cake and serve.',
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjx-AMM': {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Holiday Sugar Cookies',
    lastModified: '2010-09-18T04:08:53',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'c',
        desc: 'unsalted butter, soft',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'c', desc: 'brown sugar' },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: '', desc: 'large egg' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'Mexican vanilla',
      },
      { isGroupHeader: false, qty: 2.667, qty2: 0, uom: 'c', desc: 'flour' },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tsp',
        desc: 'baking powder',
      },
      { isGroupHeader: false, qty: 0.5, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'nutmeg (opt.)',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'tsp',
        desc: 'cinnamon (opt.)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: '',
        desc: 'vanilla bean (opt.)',
      },
    ],
    categories: 'desserts',
    yield: '~36 cookies',
    equipment: '',
    source: 'Epicurious.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjx-AMM',
    directions:
      'Cream butter & sugar.  Add egg and vanilla.  Sift in remaining ingredients and mix.  Divide, chill ~3 hours, roll dough 1/4" and cut out cookies.  Bake at 350° for 11 minutes.',
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi12wUM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Jalapeño-Goat-Cheese Hush Puppies',
    lastModified: '2010-09-18T04:10:59',
    notes:
      'Can be made 2 hours ahead. Let stand at room temperature. Rewarm until crisp in 375°F oven, 12 to 14 minutes.',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'yellow cornmeal',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'all purpose flour',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'sugar',
      },
      {
        isGroupHeader: false,
        qty: 0.75,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'coarse kosher salt',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking powder',
      },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'baking soda',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'buttermilk',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'beaten egg',
      },
      {
        isGroupHeader: false,
        qty: 1.5,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'chopped seeded jalapeño chile',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: 'coarsely crumbled soft fresh goat cheese (about 1 cup)',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'Canola oil or vegetable oil (for deep-frying)',
      },
    ],
    categories: '',
    yield: 'Makes about 18',
    equipment: '',
    source:
      'Bon Appétit, September 2010: From Caseus Fromagerie Bistro in New Haven, CT',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi12wUM',
    directions:
      'Whisk first 6 ingredients in medium bowl to blend. Whisk buttermilk, egg, and chile in small bowl to blend. Stir buttermilk mixture and cheese into dry ingredients.\n\nAdd enough oil to deep medium saucepan to reach 1 1/2 inches. Attach deep-fry thermometer to pan; heat oil to 320°F to 330°F over medium heat. Working in batches of 4 or 5, drop batter by tablespoonfuls into oil. Cook until golden, turning occasionally, about 4 minutes. Using slotted spoon, transfer hush puppies to paper towels.\n\nOriginal URL: http://www.epicurious.com/recipes/food/views/Jalapeno-Goat-Cheese-Hush-Puppies-360769',
    preptime: '30 minutes',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjW_QQM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: true,
    title: 'Malted Milk Chocolate Cheesecake',
    lastModified: '2010-09-18T04:09:42',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'tablespoon',
        desc: 'unsalted butter, softened',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Cookie crust' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc:
          'One 10-ounce package butter biscuits, wafers, or shortbread cookies',
      },
      {
        isGroupHeader: false,
        qty: 0.333,
        qty2: 0,
        uom: 'cup',
        desc: 'walnuts',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'granulated sugar',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'ounces',
        desc: '(8 tablespoons, 1 stick) unsalted butter, melted and cooled',
      },
      { isGroupHeader: true, qty: 0, qty2: 0, uom: '', desc: 'Cheesecake' },
      {
        isGroupHeader: false,
        qty: 14,
        qty2: 0,
        uom: 'ounces',
        desc: 'milk chocolate, finely chopped',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'heavy whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'malted milk powder',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'pounds',
        desc: 'cream cheese, softened',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'granulated sugar',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: 'extra-large eggs, at room temperature',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'cup', desc: 'sour cream' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'pure vanilla extract',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'quart',
        desc: 'boiling water',
      },
    ],
    categories: 'desserts, cheesecakes',
    yield: 'Makes one 9 1/2-inch round cake, 12 to 14 servings',
    equipment:
      '9 1/2-inch round springform pan, heavy-duty aluminum foil, food processor or rolling pin, double boiler, small saucepan, whisk, electric stand mixer with flat beater attachment, or large mixing bowl and handheld mixer, two rubber spatulas, fine-mesh strainer, 12-inch round cake pan or large roasting pan, cooling rack',
    source:
      'Epicurious, November 2007 (from The Essential Baker by Carole Bloom)',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhjW_QQM',
    directions:
      "Center a rack in the oven and preheat it to 300°F.\n\nUsing a paper towel or your fingertips, generously butter the inside of the springform pan. Use a double layer of heavy-duty aluminum foil to wrap tightly around the bottom of the pan. This prevents any water from the water bath seeping into the pan.\n\nCookie crust\nIn the work bowl of a food processor fitted with the steel blade, combine the cookies, walnuts, and sugar and pulse until the cookies are finely ground, about 2 minutes. Or place the cookies in a sturdy plastic bag and seal it. Use a rolling pin to crush the cookies to a very fine crumb consistency. Finely chop the walnuts and add them with the sugar to the cookies, seal the bag, and shake to blend together evenly.\n\nTransfer the cookie crumb mixture to a medium bowl and add the butter. Use a rubber spatula or a spoon to toss the mixture together to moisten all of the cookie crumbs.\n\nTransfer the crust to the prepared springform pan and use your fingertips to press it into the bottom and partway up the sides of the pan. Chill the crust while preparing the cheesecake batter.\n\nCheesecake\nPlace the milk chocolate in the top of a double boiler over low heat. Stir often with a rubber spatula to help melt evenly. Remove the top pan of the double boiler and wipe the bottom and sides very dry.\n\nOr place the milk chocolate in a microwave-safe bowl and melt on low power for 30-second bursts. Stir with a rubber spatula after each burst.\n\nPlace the cream in a small saucepan and bring to a boil over medium heat. Turn off the heat, add the malted milk powder, and whisk to distribute evenly. Cover the pan and let the mixture steep for 10 minutes, then strain the cream.\n\nPlace the cream cheese in the bowl of an electric stand mixer or in a large bowl. Use the flat beater attachment or a hand-held mixer to beat the cream cheese on medium speed until it's fluffy, about 2 minutes.\n\nAdd the sugar and cream together very well. Stop occasionally and scrape down the sides and bottom of the bowl with a rubber spatula.\n\nOne at a time, add the eggs to the cream cheese mixture, beating well after each addition. At first the eggs will sit on top of the cream cheese mixture, but stop often to scrape down the sides and bottom of the mixing bowl with a rubber spatula. This will help the mixture to blend. The mixture may also look curdled as the eggs are added, but as you stop and scrape down the bowl, the mixture will smooth out.\n\nAdd the sour cream and vanilla to the cream cheese mixture and stir together to combine.\n\nAdd the strained cream to the batter and blend thoroughly, then add the melted milk chocolate and mix together.\n\nTransfer the batter into the crust in the springform pan. Use a rubber spatula to smooth and even the top.\n\nPlace the springform pan in the larger cake pan or roasting pan and set the pan on the oven rack. Carefully pour the boiling water into the bottom pan until it reaches halfway up the side of the springform pan. Baking the cake in a water bath cushions it from the heat and adds extra moisture to the oven, which keeps the top of the cake from cracking.\n\nBake the cake for 1 hour and 45 minutes, until the top is set, but jiggles slightly. Remove the pan from the oven and transfer the cheesecake to a rack.\n\nRemove the foil and let the cheesecake cool completely. Cover the top of the cheesecake with waxed paper and wrap the pan tightly with aluminum foil. Refrigerate the cake for at least 6 hours before serving. To unmold the cheesecake, dip a thin-bladed knife in hot water and dry, then run it around the inner edge of the pan. Release the clip on the rim of the pan and gently lift it off the cake.\n\nKeeping\nStore the cheesecake, covered with waxed paper and tightly wrapped with aluminum foil, in the refrigerator up to 4 days. To freeze up to 2 months, wrap the cake pan tightly in several layers of plastic wrap and aluminum foil. Use a large piece of masking tape and an indelible marker to label and date the contents. If frozen, defrost overnight in the refrigerator and bring to room temperature at least 30 minutes before serving.\n\nMaking a change\nAdd 1 1/2 cups finely chopped walnuts to the batter before turning it into the pan.\n\nAdding style\nDrizzle the top of the cooled and chilled cheesecake with fine lines or designs of milk chocolate.\n\nServe each slice of cheesecake with a dollop of lightly sweetened whipped cream and scatter about 1 teaspoon of shaved milk chocolate over the cream.",
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj5ogMM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: true,
    title: 'Marshmallows',
    lastModified: '2010-10-15T11:18:50',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'powdered sugar',
      },
      {
        isGroupHeader: false,
        qty: 3.5,
        qty2: 0,
        uom: '',
        desc: 'envelopes / 2 tbsp + 2 1/2 tsp gelatin',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'cold water',
      },
      { isGroupHeader: false, qty: 2, qty2: 0, uom: 'cup', desc: 'sugar' },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'lt. corn syrup',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'cup',
        desc: 'hot water (~115°)',
      },
      { isGroupHeader: false, qty: 0.25, qty2: 0, uom: 'tsp', desc: 'salt' },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc: 'large egg whites',
      },
      { isGroupHeader: false, qty: 1, qty2: 0, uom: 'tsp', desc: 'vanilla' },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'colors or flavors',
      },
    ],
    categories: 'desserts',
    yield: '~96',
    equipment: '',
    source: 'Epicurious.com',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhj5ogMM',
    directions:
      'Oil bottom and sides of 13x9x2" pan and dust w/ powdered sugar.\n\nIn large bowl, sprinkle gelatin over cold water.  Soften.\n\nIn 3 qt. saucepan, cook sugar, syrup, hot water & salt over low heat, stirring with wooden spoon until sugar dissolves.  Increase heat to moderate and boil (do not stir) to 240°, ~12 minutes.\n\nPour mix over gelatin.  Stir to dissolve.  Beat on high until tripled, ~6-10 min.\n\nBeat whites to stiff peak.  Add eggs and vanilla to syrup mixture.  Beat until just combined.\n\nPour into pan.  sift 1/4 cup powdered sugar over.  Chill 3 hours to 1 day.\n\nFlip out, cut and toss in powdered sugar.  Will keep 1 week.',
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
  agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi45wEM: {
    status: 'active',
    subtitle: '',
    canEmailOwner: true,
    isFavorite: false,
    title: 'Tiramisu Eggnog Trifle',
    lastModified: '2010-09-18T04:10:30',
    notes: '',
    ingredients: [
      {
        isGroupHeader: false,
        qty: 1.333,
        qty2: 0,
        uom: 'cups',
        desc: 'plus 2 tablespoons sugar',
      },
      { isGroupHeader: false, qty: 1.25, qty2: 0, uom: 'cups', desc: 'water' },
      {
        isGroupHeader: false,
        qty: 0.25,
        qty2: 0,
        uom: 'cup',
        desc: 'plus 1 teaspoon dark rum',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'brandy',
      },
      {
        isGroupHeader: false,
        qty: 12,
        qty2: 0,
        uom: '',
        desc: 'large egg yolks',
      },
      {
        isGroupHeader: false,
        qty: 0.5,
        qty2: 0,
        uom: 'teaspoon',
        desc: 'ground nutmeg',
      },
      {
        isGroupHeader: false,
        qty: 4,
        qty2: 0,
        uom: '',
        desc: '8-ounce containers mascarpone cheese',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'cups',
        desc: 'chilled whipping cream',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'vanilla extract',
      },
      {
        isGroupHeader: false,
        qty: 6.5,
        qty2: 0,
        uom: 'teaspoons',
        desc: 'instant espresso powder',
      },
      {
        isGroupHeader: false,
        qty: 7,
        qty2: 0,
        uom: 'tablespoons',
        desc: 'Kahlúa or other coffee liqueur',
      },
      {
        isGroupHeader: false,
        qty: 2,
        qty2: 0,
        uom: '',
        desc:
          '6.15-ounce or four 3.5-ounce boxes Champagne biscuits or Boudoirs (about 60 crisp ladyfinger cookies)',
      },
      {
        isGroupHeader: false,
        qty: 1,
        qty2: 0,
        uom: 'cup',
        desc: 'semisweet chocolate chips, finely ground in processor',
      },
      {
        isGroupHeader: false,
        qty: 0,
        qty2: 0,
        uom: '',
        desc: 'chocolate leaves',
      },
    ],
    categories: 'desserts',
    yield: 'Makes 16 to 18 servings',
    equipment: '',
    source: 'Bon Appétit, December 2003',
    key: 'agtzfmJjb29rYm9va3IPCxIHUmVjaXBlMhi45wEM',
    directions:
      'Whisk 1 1/3 cups sugar, 1/4 cup water, 1/4 cup rum, 3 tablespoons brandy, yolks, and nutmeg in metal bowl. Set bowl over saucepan of simmering water (do not allow bottom of bowl to touch water). Whisk constantly until mixture thickens and candy thermometer inserted into mixture registers 140°F for 3 minutes, about 5 minutes total. Remove bowl from over water. Whisk mascarpone, 1 container at a time, into warm custard until blended.\n\nUsing electric mixer, beat whipping cream, vanilla, 1 tablespoon brandy, and 1 teaspoon rum in large bowl until cream holds peaks. Fold in mascarpone mixture.\n\nBring 1 cup water to simmer in small saucepan. Remove from heat. Add 2 tablespoons sugar and espresso powder; stir to dissolve. Mix in liqueur. Submerge 1 biscuit in espresso mixture, turning to coat twice; shake excess liquid back into pan. Place dipped biscuit, sugared side facing out, around bottom side of 14-cup trifle dish, pressing against side of dish (biscuit may break). Repeat with enough biscuits to go around bottom sides of dish once. Dip more biscuits and arrange over bottom of dish to cover.\n\nSpoon 2 cups mascarpone mixture over biscuits; spread to cover. Sprinkle 1/4 cup ground chocolate over, making chocolate visible at sides of dish. Repeat with more biscuits dipped into espresso mixture, mascarpone mixture, and ground chocolate in 2 more layers each. Cover with 1 more layer of dipped biscuits and enough mascarpone mixture to reach top of trifle dish. Sprinkle remaining ground chocolate over, covering completely. Cover and chill overnight.\n\nGently press stem end of largest chocolate leaves around edge of trifle. Fill center with smaller leaves. (Can be made 8 hours ahead; chill.)',
    preptime: '',
    owner_id: '113270077823195105196',
    owner_email: 'jakeboone02@gmail.com',
  },
};

export default recipes;
